/*
	File Name	: sp_pisc_select_item_model.SQL
	SYSTEM		: KDAC 통합 생산 정보 시스템
	Procedure Name	: sp_pisc_select_item_model
...
If Exists (Select * From sysobjects 
	    Where id = object_id(N'[dbo].[sp_pisc_select_item_model]')
	      And OBJECTPROPERTY(id, N'IsProcedure') = 1)
	Drop Procedure [dbo].[sp_pisc_select_item_model]
GO


/*
Execute sp_pisc_select_item_model 'J', 'M', '%', '%', '%'
select * from tmstmodelgroup

update tmstitem
set itemname = Rtrim(LTrim(itemName))
*/

Create Procedure sp_pisc_select_item_model
...
Select	AreaCode		= A.AreaCode,			-- 지역코드
  ...
  From	vmstmodel	A
Where	A.DivisionCode		Like @ps_divisioncode		And
