...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_eisr120i_01]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_eisr120i_01]
GO

------------------------------------------------------------------
--      업체,월별 외주간판준수율(공장별)
------------------------------------------------------------------
CREATE PROCEDURE sp_eisr120i_01
	@ps_year		Char(4),	-- 년도
	@ps_area		Char(1),	-- 지역
...
FROM (	-- M-1 TPARTKBHIS 에서 1~12월 정규간판매수계산
	Select	AreaCode, DivisionCode, SupplierCode,
	 ...
	 Where	AreaCode 		= @ps_area		And
		DivisionCode 		= @ps_division		And
		PartKBGubun		= 'N'			And
		PartKBStatus		= 'C'			And
		SubString( (Case OrderChangeFlag When 'Y' Then ChangeForecastDate Else PartForecastDate End), 1, 4) =  @ps_year 
	Group By AreaCode, DivisionCode, SupplierCode, SubString( (Case OrderChangeFlag When 'Y' Then ChangeForecastDate Else PartForecastDate End), 1, 7)
...
--		Case OrderChangeFlag When 'Y' Then ChangeForecastTime Else PartForecastTime End < @ldt_applytime 
	...
	Union
	-- T-2  TPARTKBSTATUS에서 1~12월 시간준수간판(정규간판)매수계산
	Select	AreaCode, DivisionCode, SupplierCode, 
		0 As MonthKBQty1,
		0 As MonthKBQty2,
		0 As MonthKBQty3,
...
Union
  SELECT 	A.AreaCode,			--1
		A.DivisionCode,			--2
		'ZZZZZ'	As SupplierCode,	--3
		B.AreaName,	
		C.DivisionName,	
		'소계'	As SupplierKorName,
		A.MonthKBQty1,
		A.DateKBQty1,
		Case A.MonthKBQty1 When 0 Then 0.0 Else Round( A.DateKBQty1*1.0 / A.MonthKBQty1 * 100.0 , 2) End As Drate1,
		A.TimeKBQty1,
...
/*Union
  SELECT 	A.AreaCode,			--1
		...
		'ZZZZZ'	As SupplierCode,	--3
		B.AreaName,	
		B.AreaName	As DivisionName,	
		'합계'	As SupplierKorName,	
		A.MonthKBQty1,
		A.DateKBQty1,
		Case A.MonthKBQty1 When 0 Then 0.0 Else Round( A.DateKBQty1*1.0 / A.MonthKBQty1 * 100.0 , 2) End As Drate1,
		A.TimeKBQty1,
