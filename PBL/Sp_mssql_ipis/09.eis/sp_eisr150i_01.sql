...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_eisr150i_01]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_eisr150i_01]
GO


------------------------------------------------------------------
--      외주간판준수율 Worst10 - 시간준수율(전사) 
------------------------------------------------------------------
CREATE PROCEDURE sp_eisr150i_01
--	@ps_year		Char(4),	-- 년도
--	@ps_area		Char(1),	-- 지역
	...
	SELECT 	A.AreaCode, 
	  ...
	  FROM (-- TPARTKBHIS 에서 업체별간판매수와 준수매수를 Count한다.
		Select	AreaCode, 
			...
			SupplierCode,
			Count(*) As AllKBQty,
			Case When PartObeyTimeFlag  = 1 Then Count(*) Else 0 End As KBObeyQty
		  From	TPARTKBHIS 
		...
		Group By AreaCode, DivisionCode, SupplierCode, PartObeyTimeFlag
...
--			(Case OrderChangeFlag When 'Y' Then ChangeForecastDate Else PartForecastDate End) >= @ps_from  	  	And
--			(Case OrderChangeFlag When 'Y' Then ChangeForecastDate Else PartForecastDate End) <= @ps_to 
		Group By AreaCode, DivisionCode, SupplierCode, PartObeyTimeFlag	)  As A
