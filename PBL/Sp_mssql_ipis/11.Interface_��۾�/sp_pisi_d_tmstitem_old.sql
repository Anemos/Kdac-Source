...
Begin
	...
	Where	chgdate > @ls_date
		...
		If @ls_chgcd = 'A' 	-- 추가
			...
			-- 대구전장
			If not exists (select * from ipis..tmstitem
					where	itemcode = @ls_itno)
				Insert Into ipis..tmstitem
			...
			-- 대구기계
			If not exists (select * from [ipismac_svr\ipis].ipis.dbo.tmstitem
					where	itemcode = @ls_itno)
			...
			-- 대구공조
			If not exists (select * from [ipishvac_svr\ipis].ipis.dbo.tmstitem
					where	itemcode = @ls_itno)
			...
			-- 진천
			If not exists (select * from [ipisjin_svr].ipis.dbo.tmstitem
					where	itemcode = @ls_itno)
		...
		Else
			If @ls_chgcd = 'R' 	-- 수정
				...
				-- 대구전장
				If exists (select * from ipis..tmstitem
						where	itemcode = @ls_itno)			
					Update	ipis..tmstitem
					Set	ItemName = @ls_itnm,
						ItemSpec = @ls_spec,
						ItemUnit = @ls_xunit,
						ItemType = @ls_xtype
					Where	itemcode = @ls_itno
				...
				-- 대구기계
				If exists (select * from [ipismac_svr\ipis].ipis.dbo.tmstitem
						where	itemcode = @ls_itno)			
					Update	[ipismac_svr\ipis].ipis.dbo.tmstitem
					Set	ItemName = @ls_itnm,
						ItemSpec = @ls_spec,
						ItemUnit = @ls_xunit,
						ItemType = @ls_xtype
				...
				-- 대구공조
				If exists (select * from [ipishvac_svr\ipis].ipis.dbo.tmstitem
						where	itemcode = @ls_itno)
					Update	[ipishvac_svr\ipis].ipis.dbo.tmstitem
					Set	ItemName = @ls_itnm,
						ItemSpec = @ls_spec,
						ItemUnit = @ls_xunit,
						ItemType = @ls_xtype
				...
				-- 진천
				If exists (select * from [ipisjin_svr].ipis.dbo.tmstitem
						where	itemcode = @ls_itno)
					Update	[ipisjin_svr].ipis.dbo.tmstitem
					Set	ItemName = @ls_itnm,
						ItemSpec = @ls_spec,
						ItemUnit = @ls_xunit,
						ItemType = @ls_xtype
