/*
	File Name	: sp_pisi_eis_tmstkbhis.SQL
	SYSTEM		: KDAC 통합 생산 정보 시스템
	Procedure Name	: sp_pisi_eis_tmstkbhis
...
If Exists (Select * From sysobjects 
	    Where id = object_id(N'[dbo].[sp_pisi_eis_tmstkbhis]')
	      And OBJECTPROPERTY(id, N'IsProcedure') = 1)
	Drop Procedure [dbo].[sp_pisi_eis_tmstkbhis]
GO

/*
Execute sp_pisi_eis_tmstkbhis
*/

Create Procedure sp_pisi_eis_tmstkbhis
...
-- 각 서버 tdelete에서 tablename이 tmstkbhis인 놈 정리하고...

select	@ls_yesterday = convert(char(10), getdate() - 1, 102)

-- 대구전장	

Insert	into	#tmp_tmstkbhis
select	*
from	[ipisele_svr\ipis].ipis.dbo.tmstkbhis
where	lastemp = 'Y'
...
Begin
	...
	If not exists (select * from tmstkbhis 
			...
			and	ApplyTo		= @ls_applyto)

		insert into tmstkbhis
			(AreaCode,	DivisionCode,		WorkCenter,	LineCode,	ItemCode,
			ApplyFrom,	ApplyTo,		ModelID,	LineID,		KBID,		
			NormalKBSN,	TempKBSN,		ProductGubun,			StockGubun,	
			PrdStopGubun,		RackCode,	RackQty,	LotSize,	CarName,
			MainLineGubun,		DivideRate,	PCSPerHour,	SafetyInvQty,	
			KBFactor,	SafetyFactor,		StockLocation,	
			SortOrder,	LastEmp)
		values	(@ls_AreaCode,	@ls_DivisionCode,	@ls_WorkCenter,	@ls_LineCode,	@ls_ItemCode,
			@ls_applyfrom,	@ls_applyto,		@ls_modelid,	@ls_lineid,	@ls_kbid,	
			@ls_normalkbsn,	@ls_tempkbsn,		@ls_productgubun,		@ls_stockgubun,	
			@ls_prdstopgubun,	@ls_rackcode,	@ls_rackqty,	@ls_lotsize,	@ls_carname,
			@ls_mainlinegubun,	@ls_dividerate,	@ls_pcsperhour,	@ls_safetyinvqty,
	...
	update	[ipisele_svr\ipis].ipis.dbo.tmstkbhis
	set	lastemp 	= 'N'
	where	AreaCode	= @ls_areacode
	and	DivisionCode	= @ls_divisioncode
	and	WorkCenter	= @ls_workcenter
...
Insert	into	#tmp_tmstkbhis
select	*
from	[ipismac_svr\ipis].ipis.dbo.tmstkbhis
where	lastemp = 'Y'
...
Begin
	...
	update	[ipismac_svr\ipis].ipis.dbo.tmstkbhis
	set	lastemp 	= 'N'
	where	AreaCode	= @ls_areacode
	and	DivisionCode	= @ls_divisioncode
	and	WorkCenter	= @ls_workcenter
...
Insert	into	#tmp_tmstkbhis
select	*
from	[ipishvac_svr\ipis].ipis.dbo.tmstkbhis
where	lastemp = 'Y'
...
Begin
	...
	update	[ipishvac_svr\ipis].ipis.dbo.tmstkbhis
	set	lastemp 	= 'N'
	where	AreaCode	= @ls_areacode
	and	DivisionCode	= @ls_divisioncode
	and	WorkCenter	= @ls_workcenter
...
Insert	into	#tmp_tmstkbhis
select	*
from	[ipisjin_svr].ipis.dbo.tmstkbhis
where	lastemp = 'Y'
...
Begin
	...
	update	[ipisjin_svr].ipis.dbo.tmstkbhis
	set	lastemp 	= 'N'
	where	AreaCode	= @ls_areacode
	and	DivisionCode	= @ls_divisioncode
	and	WorkCenter	= @ls_workcenter
