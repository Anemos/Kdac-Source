...
-- 부평물류
If Exists (select * from tmisbomdata	where	Mplant	= 'B'	and
						mdate		=	@ps_month)
Begin
	Delete From [ipisbup_svr\ipis].ipis.dbo.tmstbomdata
	Where	applymonth	=	@ls_Month

	if @@error	=	0
	Begin
		insert into [ipisbup_svr\ipis].ipis.dbo.tmstbomdata
			(AreaCode,	DivisionCode,	
			ApplyMonth,	
			BMDCD,		BMDNO,		BSERNO,		BPRNO,		BCHNO,
			BLEV,		BUM,		BWOCT,		BPRQT,		
...
-- 군산물류
If Exists (select * from tmisbomdata	where	Mplant	= 'K'	and
						mdate		=	@ps_month)
Begin
	Delete From [ipiskun_svr\ipis].ipis.dbo.tmstbomdata
	Where	applymonth	=	@ls_Month

	if @@error	=	0
	Begin
		insert into [ipiskun_svr\ipis].ipis.dbo.tmstbomdata
			(AreaCode,	DivisionCode,	
			ApplyMonth,	
			BMDCD,		BMDNO,		BSERNO,		BPRNO,		BCHNO,
			BLEV,		BUM,		BWOCT,		BPRQT,		
