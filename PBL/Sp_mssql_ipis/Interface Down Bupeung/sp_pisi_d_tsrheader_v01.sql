...
Begin
  ...
  Select  Top 1
    ...
    @ls_paycd = paycd,
    @ls_dstn  = dstn,
    @ls_inptdt  = substring(chgdate,1,4)+'.'+substring(chgdate,6,2)+'.'+substring(chgdate,9,2),
    @ls_extd    = substring(chgdate,12,2) + ':' + substring(chgdate,15,2),
  ...
  begin
    ...
    insert into [ipisele_svr\ipis].ipis.dbo.tsrorder
      ...
      orderno,    LastEmp,    Extd,     Luprc,  Luamt, epno)
    select  rtrim(csrno) + 'P00', 'P',      rtrim(csrno) + 'P00',
      ...
      citno,      suse,     srno,     'N',
      ordno,      'SYSTEM',   extd,     luprc,  luamt, epno
    from  Sle302_Log
    where srno = @ls_srno
    and kitcd in ('P','','M')
    and xplant = 'D'
    ...
    insert into [ipisele_svr\ipis].ipis.dbo.tsrorder
      ...
      ShipOrderQty,   ShipRemainQty,    ShipEndGubun,   SRCancelGubun,
      CustomerItemNo,   ShipUsage,    CheckSRNo,    stcd,
      orderno,    LastEmp,    Luprc,      Luamt, epno)
    select  rtrim(b.csrno)+'C00', 'C',      rtrim(b.psrno) + 'P00',
      ...
      b.ordno,      'SYSTEM', b.luprc,    b.luamt, a.epno
    ...
    If (@ls_area = 'H' or @ls_area = 'V' or @ls_area = 'N' or @ls_area = 'O' or @ls_area = 'P')
    Begin
    ...
    where srno = @ls_srno
    and kitcd in ('P','','M')
    and ((xplant = 'D' and div in ('H','V')) or xplant = 'K' )
    ...
    where a.srno = @ls_srno
    and a.csrno = b.psrno
    and ((b.xplant = 'D' and b.div in ('H','V')) or b.xplant = 'K')
    ...
    if exists (select * from [ipisyeo_svr\ipis].ipis.dbo.tsrheader
        Where checksrno = @ls_srno)
      delete  From  pdsle301_log
      where chgdate = @ls_chgdate
  ...
  Begin
    ...
    where a.srno = @ls_srno
    and a.csrno = b.psrno
    and (b.xplant = 'D' and b.div in ('H','V'))
    ...
    if exists (select * from [ipisbup_svr\ipis].ipis.dbo.tsrheader
        Where checksrno = @ls_srno)
      delete  From  pdsle301_log
      where chgdate = @ls_chgdate
    ...
    if exists (select * from [ipiskun_svr\ipis].ipis.dbo.tsrheader
        Where checksrno = @ls_srno)
      delete  From  pdsle301_log
      where chgdate = @ls_chgdate
  ...
  Begin
    ...
    If @ls_deletecheck = 0
    begin
      delete from [ipisbup_svr\ipis].ipis.dbo.tsrorder
      where checksrno = @ls_srno

      delete from [ipisbup_svr\ipis].ipis.dbo.tsrheader
      where checksrno = @ls_srno
    ...
    If @ls_deletecheck = 0
    begin
      delete from [ipiskun_svr\ipis].ipis.dbo.tsrorder
      where checksrno = @ls_srno

      delete from [ipiskun_svr\ipis].ipis.dbo.tsrheader
      where checksrno = @ls_srno
    ...
    If @ls_deletecheck = 0
    begin
      delete from [ipisyeo_svr\ipis].ipis.dbo.tsrorder
      where checksrno = @ls_srno

      delete from [ipisyeo_svr\ipis].ipis.dbo.tsrheader
      where checksrno = @ls_srno
