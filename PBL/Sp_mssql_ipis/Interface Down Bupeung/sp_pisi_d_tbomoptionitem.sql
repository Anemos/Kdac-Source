...
If Exists (select * From bom003)
  ...
  Begin
    ...
    select oplant, odvsn, opitn, ofitn,
      ...
      orate, ochcd, ofocd, oemno, getdate()
    from bom003
    where (oplant = 'D' and odvsn in ('H','V'))

    -- 부평물류
    exec [ipisbup_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tbomoptionitem'

    insert into [ipisbup_svr\ipis].ipis.dbo.tbomoptionitem
    (Oplant, Odvsn, Opitn, Ofitn, Ochdt, Oedtm, Oedte, Orate, Ochcd, Ofocd, Oemno, LastDate)
    select oplant, odvsn, opitn, ofitn,
      substring(ochdt, 1, 4)+'.'+substring(ochdt, 5, 2)+'.'+substring(ochdt, 7,2),
      case
    ...
    -- 군산물류
    exec [ipiskun_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tbomoptionitem'

    insert into [ipiskun_svr\ipis].ipis.dbo.tbomoptionitem
    (Oplant, Odvsn, Opitn, Ofitn, Ochdt, Oedtm, Oedte, Orate, Ochcd, Ofocd, Oemno, LastDate)
    select oplant, odvsn, opitn, ofitn,
      substring(ochdt, 1, 4)+'.'+substring(ochdt, 5, 2)+'.'+substring(ochdt, 7,2),
      case
