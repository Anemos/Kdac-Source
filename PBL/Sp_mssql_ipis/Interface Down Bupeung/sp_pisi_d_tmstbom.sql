...
If Exists (select * From tmisbom)
	...
	Begin
		...
		insert into [ipisele_svr\ipis].ipis.dbo.tmstbom
			...
			ApplyFrom,
			ApplyTo,		
			BOMAreaCode, BOMDivisionCode, CostGubun,	EBOMExistFlag,
			BOMFirstInputDate, 
			Empno,	LastEmp)
		...
		select 	plant,		pdvsn,		ppitn,		prout,		pcitn,
			...
			pemno,	'SYSTEM'
		from	tmisbom
		where	(plant = 'D' and pdvsn in ('H','V'))

		-- 부평물류
		exec [ipisbup_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tmstbom'
		
		insert into [ipisbup_svr\ipis].ipis.dbo.tmstbom
			(AreaCode,	DivisionCode,	BOMPItemNo,	Routing,		BOMCItemNo,
			BOMChangeDate,	
			BOMLUnit,	BOMEUnit,	BOMWorkCenter,	
			ApplyFrom,
    ...
    -- 군산물류
		exec [ipiskun_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tmstbom'
		
		insert into [ipiskun_svr\ipis].ipis.dbo.tmstbom
			(AreaCode,	DivisionCode,	BOMPItemNo,	Routing,		BOMCItemNo,
			BOMChangeDate,	
			BOMLUnit,	BOMEUnit,	BOMWorkCenter,	
			ApplyFrom,
