...
-- 부평물류
delete from [ipisbup_svr\ipis].ipis.dbo.tmstpartcost
where SupplierCode+ItemCode in (select rtrim(vsrno)+rtrim(itno) from #tmp_pdpur103
			where	len(vsrno)	<=	5)
...
-- 군산물류
delete from [ipiskun_svr\ipis].ipis.dbo.tmstpartcost
where SupplierCode+ItemCode in (select rtrim(vsrno)+rtrim(itno) from #tmp_pdpur103
			where	len(vsrno)	<=	5)
...
-- 여주전자 (외자)
delete from [ipisyeo_svr\ipis].ipis.dbo.tmstpartcostoversea
where SupplierCode+ItemCode in (select rtrim(vsrno)+rtrim(itno) from #tmp_pdpur103
			where	len(vsrno)	>	5)
