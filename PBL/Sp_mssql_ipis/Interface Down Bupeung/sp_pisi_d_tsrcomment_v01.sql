...
Begin
	...
	Select	Top 1
		...
		@ls_srno	= rtrim(srno),
		@ls_gubun	= gubun,
		@ls_comment	= cast(comment as varchar(1000)),
	...
	If @ls_area = 'B'
		-- 부평물류
		If not exists (select * from [ipisbup_svr\ipis].ipis.dbo.tsrcomment
					where	checksrno = @ls_srno and gubun =  @ls_gubun)
			begin
			insert into [ipisbup_svr\ipis].ipis.dbo.tsrcomment
				(CheckSRNo,		Gubun,			SRComment,		LastEmp)
			values	(@ls_srno,		@ls_gubun,		@ls_comment,		'SYSTEM')
			end
			
		If exists (select * from [ipisbup_svr\ipis].ipis.dbo.tsrcomment
					where	checksrno = @ls_srno and gubun =  @ls_gubun)
			begin
			Delete	From	pdsle305_log
			Where	chgdate	=	@ls_chgdate
	...
	If @ls_area = 'K'
		-- 군산물류
		If not exists (select * from [ipiskun_svr\ipis].ipis.dbo.tsrcomment
					where	checksrno = @ls_srno and gubun =  @ls_gubun)
			begin
			insert into [ipiskun_svr\ipis].ipis.dbo.tsrcomment
				(CheckSRNo,		Gubun,			SRComment,		LastEmp)
			values	(@ls_srno,		@ls_gubun,		@ls_comment,		'SYSTEM')
			end
			
		If exists (select * from [ipiskun_svr\ipis].ipis.dbo.tsrcomment
					where	checksrno = @ls_srno and gubun =  @ls_gubun)
			begin
			Delete	From	pdsle305_log
			Where	chgdate	=	@ls_chgdate
  ...
  If @ls_area = 'B'
		-- 부평물류
		update	[ipisbup_svr\ipis].ipis.dbo.tsrcomment
		set	srcomment	= @ls_comment,
			lastdate	= Getdate()
		where	checksrno	= @ls_srno
  ...
  If @ls_area = 'K'
		-- 군산물류
		update	[ipiskun_svr\ipis].ipis.dbo.tsrcomment
		set	srcomment	= @ls_comment,
			lastdate	= Getdate()
		where	checksrno	= @ls_srno
  ...
  If @ls_area = 'Y'
	...
	If @ls_area = 'B'
		-- 부평물류
		delete from [ipisbup_svr\ipis].ipis.dbo.tsrcomment
		where	checksrno = @ls_srno
	...
	If @ls_area = 'K'
		-- 군산물류
		delete from [ipiskun_svr\ipis].ipis.dbo.tsrcomment
		where	checksrno = @ls_srno
