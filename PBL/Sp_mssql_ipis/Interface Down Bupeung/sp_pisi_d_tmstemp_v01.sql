...
If Exists (Select * From tmisemp)
	Begin
		...
		-- 부평물류
		exec [ipisbup_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tmstemp'
		
		insert into [ipisbup_svr\ipis].ipis.dbo.tmstemp
		select *
		from	[ipisele_svr\ipis].ipis.dbo.tmstemp
		
		-- 군산물류
		exec [ipiskun_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tmstemp'
		
		insert into [ipiskun_svr\ipis].ipis.dbo.tmstemp
		select *
		from	[ipisele_svr\ipis].ipis.dbo.tmstemp
