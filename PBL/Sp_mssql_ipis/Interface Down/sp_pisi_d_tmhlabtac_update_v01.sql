...
-- 부평물류
  ...
  from [ipis_daegu].interface.dbo.tmislabtac_log a inner join [ipisbup_svr\ipis].ipis.dbo.tmstdept b
	on a.dgdepte = b.deptcode
    inner join [ipisbup_svr\ipis].ipis.dbo.tmstemp c
	on a.empno = c.empno
  where b.areacode in ('X','B') and
    b.divisioncode in ('X','A','M','S','H','V') and
    not exists ( select * from [ipisbup_svr\ipis].ipis.dbo.tmhlabtac d
...
-- 군산물류
  ...
  Select  a.empno,  a.dgyy+'.'+a.dgmm+'.'+a.dgdd, a.dgdiv,    a.dgdepte,
    ...
    a.dgclass,  c.empjikchek, a.dgindt,   a.dgintime, a.dginusr,
    a.dgupdt,   a.dguptime, a.dgupusr,  null 
  from [ipis_daegu].interface.dbo.tmislabtac_log a inner join [ipiskun_svr\ipis].ipis.dbo.tmstdept b
	on a.dgdepte = b.deptcode
    inner join [ipiskun_svr\ipis].ipis.dbo.tmstemp c
	on a.empno = c.empno
  where b.areacode in ('X','K') and
    b.divisioncode in ('X','M','S','H') and
    not exists ( select * from [ipiskun_svr\ipis].ipis.dbo.tmhlabtac d
		where a.EMPNO = d.DGEMPNO and (a.DGYY + a.DGMM + a.DGDD) = CONVERT(char(8),cast(d.DGDAY as DATETIME),112))
...
-- 여주전자
  ...
  Select  a.empno,  a.dgyy+'.'+a.dgmm+'.'+a.dgdd, a.dgdiv,    a.dgdepte,
    ...
    a.dgclass,  c.empjikchek, a.dgindt,   a.dgintime, a.dginusr,
    a.dgupdt,   a.dguptime, a.dgupusr,  null 
  from [ipis_daegu].interface.dbo.tmislabtac_log a inner join [ipisyeo_svr\ipis].ipis.dbo.tmstdept b
	on a.dgdepte = b.deptcode
    inner join [ipisyeo_svr\ipis].ipis.dbo.tmstemp c
	on a.empno = c.empno
  where b.areacode in ('X','D','Y') and
    b.divisioncode in ('X','Y') and
    not exists ( select * from [ipisyeo_svr\ipis].ipis.dbo.tmhlabtac d
		where a.EMPNO = d.DGEMPNO and (a.DGYY + a.DGMM + a.DGDD) = CONVERT(char(8),cast(d.DGDAY as DATETIME),112))
...
-- 진천
  ...
  from [ipis_daegu].interface.dbo.tmislabtac_log a inner join [ipisjin_svr].ipis.dbo.tmstdept b
	on a.dgdepte = b.deptcode
    inner join [ipisjin_svr].ipis.dbo.tmstemp c
	on a.empno = c.empno
  where b.areacode in ('X','J') and
    not exists ( select * from [ipisjin_svr].ipis.dbo.tmhlabtac d
		where a.EMPNO = d.DGEMPNO and (a.DGYY + a.DGMM + a.DGDD) = CONVERT(char(8),cast(d.DGDAY as DATETIME),112))
