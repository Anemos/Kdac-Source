...
If Exists (select * From tmisrouting)
  Begin
    ...
    exec [ipis_daegu].ipis.dbo.sp_pisi_truncate_table 'tmstrouting'
    -- main routing
    insert into [ipis_daegu].ipis.dbo.tmstrouting
    (AreaCode, DivisionCode, ItemCode, SubLineCode, SubLineNo,
      ...
      SideMCWorkTime, SideManualWorkTime, EmpCount, LastEmp, BasePower)
    select  rcplant, rcdvsn, rcitno,  upper(rcline1), rcline2,
      ...
      rcnvmc,     rcnvlb,     rclbcnt,    'SYSTEM', rcpower
    from  tmisrouting
    where rcplant = @ls_plant
    and rcdvsn = @ls_dvsn and rcline1 <> 'DUMMY'
    
    -- sub routing
    select distinct rcitno = a.rcitno, rcedto = a.rcedto into #tmp_master_ele
    from tmisrouting a
    where a.rcplant = @ls_plant and a.rcdvsn = @ls_dvsn and  a.rcline1 <> 'DUMMY' and
      exists ( select * from tmisrouting_subitem b 
          where a.rcplant = b.raplant and a.rcdvsn = b.radvsn and 
            a.rcitno = b.raitno and a.rcitno <> b.raitno1 )
    ...
    -- set 군산공장 plant,dvsn
    select @ls_plant = 'K'
    exec [ipiskun_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tmstrouting'
    -- main routing
    insert into [ipiskun_svr\ipis].ipis.dbo.tmstrouting
    (AreaCode, DivisionCode, ItemCode, SubLineCode, SubLineNo,
      ProcessNo, ApplyDate,
      Enddate,
      ProcessName, ProcessSeq, WorkCenter, GradeGubun, MCGubun,
    ...
    select  rcplant, rcdvsn, rcitno,  upper(rcline1), rcline2,
      ...
      rcnvmc,     rcnvlb,     rclbcnt,    'SYSTEM', rcpower
    from  tmisrouting
    where rcplant = @ls_plant and rcline1 <> 'DUMMY'

    -- sub routing
    select distinct rcitno = a.rcitno, rcedto = a.rcedto into #tmp_master_kun
    from tmisrouting a
    where a.rcplant = @ls_plant and a.rcline1 <> 'DUMMY' and
      exists ( select * from tmisrouting_subitem b 
          where a.rcplant = b.raplant and a.rcdvsn = b.radvsn and 
            a.rcitno = b.raitno and a.rcitno <> b.raitno1 )
    ...
    While @@error = 0
      ...
      drop table #tmp_master_kun
      
    -- set 부평공장 plant
    select @ls_plant = 'B'
    exec [ipisbup_svr\ipis].ipis.dbo.sp_pisi_truncate_table 'tmstrouting'
    -- main routing
    insert into [ipisbup_svr\ipis].ipis.dbo.tmstrouting
    (AreaCode, DivisionCode, ItemCode, SubLineCode, SubLineNo,
      ProcessNo, ApplyDate,
      Enddate,
      ProcessName, ProcessSeq, WorkCenter, GradeGubun, MCGubun,
    ...
    -- sub routing
    select distinct rcitno = a.rcitno, rcedto = a.rcedto into #tmp_master_bup
    from tmisrouting a
    where a.rcplant = @ls_plant and a.rcline1 <> 'DUMMY' and
      exists ( select * from tmisrouting_subitem b 
          where a.rcplant = b.raplant and a.rcdvsn = b.radvsn and 
