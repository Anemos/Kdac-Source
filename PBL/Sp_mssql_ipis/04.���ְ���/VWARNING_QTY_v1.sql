...
From ( select areacode, divisioncode, itemcode, plandate, sum(changeqty) as changeqty, sum(0) as releaseqty
	...
	group by areacode, divisioncode, itemcode, plandate 
	union all
	select plandate, areacode, divisioncode, itemcode, sum(0) as changeqty,Sum( ReleaseKbCount * ReleaseKbQty ) as Releaseqty
	from tplanrelease
