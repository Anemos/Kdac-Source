...
select aa.areacode, aa.divisioncode, aa.itemcode, aa.suppliercode,
  aa.parttype, aa.rackqty, 0
from tmstpartkbhis aa inner join ( select areacode, divisioncode, itemcode from tmstpartkbhis
...
while @li_cnt <= (@li_totcnt - 1)
  Begin
    ...
    insert into #tmp_supplier
    select top 1 areacode, divisioncode, itemcode, suppliercode,
      parttype, rackqty, 1
    from tmstpartkbhis
    where areacode = @ls_areacode and divisioncode = @ls_divisioncode and
      itemcode = @ls_itemcode and applyto >= convert(char(10),getdate(),102)
