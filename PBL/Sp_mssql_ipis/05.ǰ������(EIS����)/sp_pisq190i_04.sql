/************************************************************************************************/
/*	File Name	: sp_pisq190i_04.SQL                                                            */
/*	SYSTEM		: 품질관리(공정품질)                                                            */
/*	Description	: 월별 FTTQ 현황(제품소분류별공정별)                                            */
...
/*	Notes		: 월간 FTTQ현황중 소분류별공정별 자료를 조회한다.                               */
/*	Made Date	: 2002. 09. 26                                                                  */
/*	Author		: 대우정보-유종희                                                               */
/************************************************************************************************/
if exists (select * from sysobjects where id = object_id('dbo.sp_pisq190i_04') and sysstat & 0xf = 4)
	drop procedure dbo.sp_pisq190i_04
GO
/*
Exec sp_pisq190i_04
		@ps_AreaCode        = 'D' ,
...
/****** Object:  Stored Procedure dbo.sp_pisq190i_04    Script Date: 02-09-01  ******/
CREATE PROCEDURE sp_pisq190i_04
	  ...
	  FROM (SELECT '#00'    AS AS_PROCESS,
		    ...
		    SELECT A.PROCESSCODE       AS AS_PROCESSCODE,
			 ...
			 WHERE A.AREACODE		 =  B.AREACODE
			   ...
			   AND A.SMALLGROUPCODE  =  @ps_smallgroupcode
			   AND B.MAFLAG          LIKE @ps_maflag
			 GROUP BY A.PROCESSCODE, D.PROCESSNAME, AS_YEARGOAL, M_BSTOT, M_S11, M_S10, M_S09, M_S08,
			 	      M_S07, M_S06, M_S05, M_S04, M_S03, M_S02, M_S01, M_S00,M_STOT
	       ) TMP
