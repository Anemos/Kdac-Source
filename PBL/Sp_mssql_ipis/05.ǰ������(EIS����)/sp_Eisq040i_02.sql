/************************************************************************************************/
/*	File Name	: sp_eisq040i_02.SQL                                                            */
/*	SYSTEM		: 경영자용 통합정보	                                                            */
/*	Description	: 전사 월간 수입검사 불량율(업체별WORST-LOT) 	                                */
...
/*	Author		: 대우정보-유종희                                                               */
/************************************************************************************************/
if exists (select * from sysobjects where id = object_id('dbo.sp_eisq040i_02') and sysstat & 0xf = 4)
	drop procedure dbo.sp_eisq040i_02
GO
/*
Exec sp_eisq040i_02
...
/****** Object:  Stored Procedure dbo.sp_eisq040i_02    Script Date: 02-09-01  ******/
CREATE PROCEDURE sp_eisq040i_02
	...
	BEGIN
		...
		BEGIN   
			...
			SELECT	TOP 10
					...
					AS_SUPPLIERCODE		AS_SUPPLIER,
					sum(ISNULL(AS_SUPPLIERDELIVERYQTY, 0))	AS AS_IQTY,
					sum(ISNULL(AS_BADQTY, 0))				AS AS_BQTY,
					CASE WHEN sum(ISNULL(AS_SUPPLIERDELIVERYQTY,0)) = 0 
						  THEN	0
						  ELSE	 ISNULL((cast(SUM(AS_BADQTY) as float) / cast(sum(AS_SUPPLIERDELIVERYQTY) as float)) * 100 , 0) 
						  END 	AS AS_RATE
			  FROM (
			...
			GROUP BY AS_AREACODE, AS_DIVISIONCODE, AS_SUPPLIERCODE
			ORDER BY AS_AREACODE, AS_DIVISIONCODE, AS_RATE DESC 
