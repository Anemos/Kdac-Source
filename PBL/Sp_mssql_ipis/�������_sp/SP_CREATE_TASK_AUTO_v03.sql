...
CREATE   PROCEDURE SP_CREATE_TASK_AUTO
AS
...
Declare   @li_TotCnt    AS Int
  ...
  Select Distinct A.Equip_Code, A.Class_Div, B.Area_Code, B.Factory_Code, A.Class_Est_Date
  ...
  Begin
    ...
    values(@NextWoNo, @Area, @Factory, @EquipCode, @EstDate, '계획')

    UPDATE Equip_Class
    SET Class_Est_Date =  case When Cycle_Code = '년' then DateAdd(YEAR, (1 * Class_Cycle), @EstDate)
      When Cycle_Code = '반기' then DateAdd(MONTH, (6 * Class_Cycle), @EstDate)
      When Cycle_Code = '분기' then DateAdd(MONTH, (3 * Class_Cycle), @EstDate)
      When Cycle_Code = '월' then DateAdd(MONTH, (1 * Class_Cycle), @EstDate)
      When Cycle_Code = '주' then DateAdd(WEEK, (1 * Class_Cycle), @EstDate)
      When Cycle_Code = '일' then DateAdd(DAY, (1* Class_Cycle), @EstDate)
      Else Class_Est_Date End
    Where Equip_Code = @EquipCode And Class_Div = @Div
        And Class_Est_Date = @EstDate And Class_Div <> '04'
  End
