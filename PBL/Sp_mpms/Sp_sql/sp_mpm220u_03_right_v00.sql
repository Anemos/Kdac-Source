/*
  File Name : sp_mpm220u_03_right.SQL
  SYSTEM    : 금형공정관리 시스템
  Procedure Name  : sp_mpm220u_03_right
...
If Exists (Select * From sysobjects
      Where id = object_id(N'[dbo].[sp_mpm220u_03_right]')
        And OBJECTPROPERTY(id, N'IsProcedure') = 1)
  Drop Procedure [dbo].[sp_mpm220u_03_right]
GO

/*
Execute sp_mpm220u_03_right '@ps_orderno', '@ps_partno'
*/

Create Procedure sp_mpm220u_03_right
  @ps_orderno char(8),
  @ps_partno char(6)
...
insert into #tmp_bottomdivide(printdate,orderno,toolname,partno,
...
Where cc.OrderNo = @ps_orderno and cc.PartNo = @ps_partno and
  ...
  cast(bb.serialno as varchar(10)) + cc.PartNo + cc.Operno not in ( Select TOP 50 PERCENT cast(ee.serialno as varchar(10)) + dd.PartNo + dd.Operno
									...
									Where dd.OrderNo = @ps_orderno and dd.PartNo = @ps_partno and
									      dd.WcCode <> 'THT' and dd.OutFlag <> 'P'
									order by dd.orderno, ee.serialno, dd.partno, dd.operno)
order by aa.orderno, bb.serialno, cc.partno, cc.operno
...
Begin
  insert into #tmp_bottomresult
  select printdate,orderno,toolname,partno,
  partname,planqty,material,operno,wccode,postwccode,
  stdtime,workmatter,workstart,workemp,workempname,mchno,
  workdate,begintime,endtime,finalqty,scrapqty,jobtime
  from #tmp_bottomdivide where serialno = @li_cntx
  ...
  if @li_cntk <= @li_rowcntk
    Begin
      ...
      stdtime,workmatter,workstart,workemp,workempname,mchno,
      workdate,begintime,endtime,finalqty,scrapqty,jobtime
      from #tmp_bottomdivide where serialno = @li_cntk

      select @li_cntk = @li_cntk + 1
    End
End

select * from #tmp_bottomresult
drop table #tmp_bottomdivide
drop table #tmp_bottomresult
Return
