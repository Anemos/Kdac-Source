...
FROM ( select stype = cc.stype,
  ...
  from trouting aa inner join tworkcenter bb
    ...
    on aa.orderno = cc.orderno and aa.partno = cc.partno and aa.operno = cc.operno
  where bb.wccode = @ps_linecode and bb.wgcode = @ps_workcenter and 
        aa.workstatus <> 'C'
  group by cc.stype, aa.orderno, aa.partno, aa.operno, convert(char(10),aa.workstart,102)
  ) tmp
WHERE tmp.preworkstatus = 'C'
) tmp_result
ORDER BY tmp_result.as_lastdate DESC, tmp_result.as_processratio DESC
