/*
  ...
  적용년월에 완료된 Order에 대한 총 누적을 보여줌.( 시작일부터 완료일까지 )
  Use DataBase  : MPMS
  Use Program :
...
Begin
  ...
  from ( select bb.deptname as deptname, aa.orderdept as orderdept,
      sum(isnull(cc.HeatCost,0) + isnull(cc.OutCost,0) 
        + isnull(cc.MatCost,0) + isnull(dd.SubMatCost,0)) as matcost,
      ...
      sum(isnull(cc.HeatCost,0) + isnull(cc.OutCost,0) + isnull(cc.MatCost,0)
        + isnull(cc.ManCost,0) + isnull(cc.MchCost,0) + isnull(dd.SubMatCost,0)) as sumcost,
      sum(convert(decimal(10,0),(isnull(cc.HeatCost,0) + isnull(cc.OutCost,0) + isnull(cc.MatCost,0)
        + isnull(cc.ManCost,0) + isnull(cc.MchCost,0) + isnull(dd.SubMatCost,0)) * 1.2)) as totcost,
      sum(0) as matcost1,
      sum(0) as mancost1,
      sum(0) as mchcost1,
      sum(0) as sumcost1,
      ...
      from torder aa left outer join tmstdept bb
        ...
        on aa.orderno = cc.orderno and
          cc.yearmm >= convert(char(6),convert(datetime,@ps_fromdt),112) and
          cc.yearmm <= convert(char(6),convert(datetime,@ps_todt),112)
        left outer join tsubmatcost dd
        on aa.orderdept = dd.orderdept and
          dd.yearmm >= convert(char(6),convert(datetime,@ps_fromdt),112) and
          dd.yearmm <= convert(char(6),convert(datetime,@ps_todt),112)
      where convert(char(10),aa.startdate,102) >= @ps_fromdt and 
        ( aa.enddate is not null and convert(char(10),aa.enddate,102) <= @ps_todt ) and
      ...
      from tsubmatcost aa left outer join tmstdept bb
        on aa.orderdept = bb.deptcode and
          aa.yearmm >= convert(char(6),convert(datetime,@ps_fromdt),112) and
          aa.yearmm <= convert(char(6),convert(datetime,@ps_todt),112)
      where aa.orderdept not in ( select orderdept from torder 
          where convert(char(10),startdate,102) >= @ps_fromdt and 
            ( enddate is not null and convert(char(10),enddate,102) <= @ps_todt ) and
              deptgubun = '1' and assetflag = 'E' )
      group by bb.deptname, aa.orderdept
      ...
      sum(0) as totcost,
      sum(isnull(cc.HeatCost,0) + isnull(cc.OutCost,0) 
        + isnull(cc.MatCost,0) + isnull(dd.SubMatCost,0)) as matcost1,
      sum(isnull(cc.ManCost,0)) as mancost1,
      sum(isnull(cc.MchCost,0)) as mchcost1,
      sum(isnull(cc.HeatCost,0) + isnull(cc.OutCost,0) + isnull(cc.MatCost,0)
        + isnull(cc.ManCost,0) + isnull(cc.MchCost,0) + isnull(dd.SubMatCost,0)) as sumcost1,
      sum(convert(decimal(10,0),(isnull(cc.HeatCost,0) + isnull(cc.OutCost,0) + isnull(cc.MatCost,0)
        + isnull(cc.ManCost,0) + isnull(cc.MchCost,0) + isnull(dd.SubMatCost,0)) * 1.2)) as totcost1
      from torder aa left outer join tmstdept bb
        on aa.orderdept = bb.deptcode
        left outer join tmatcost cc
        on aa.orderno = cc.orderno and
