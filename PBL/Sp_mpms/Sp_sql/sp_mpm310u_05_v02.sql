/*
	File Name	: sp_mpm310u_05.SQL
	SYSTEM		: 금형공정관리 시스템
	Procedure Name	: sp_mpm310u_05
	Description	: 작업지시서 출력(품번별)
	Use DataBase	: MPMS
	Use Program	:
...
If Exists (Select * From sysobjects 
	    Where id = object_id(N'[dbo].[sp_mpm310u_05]')
	      And OBJECTPROPERTY(id, N'IsProcedure') = 1)
	Drop Procedure [dbo].[sp_mpm310u_05]
GO

/*
Execute sp_mpm310u_05 '@ps_orderno','@ps_partno'
*/

Create Procedure sp_mpm310u_05
  @ps_orderno char(8),
  @ps_partno  char(6)
...
Select PrintDate = Convert(char(10),getdate(),102),
  ...
  JobTime = ' ',
  JobMemo = ' ',
  StdTime = case cc.outflag when 'P' then null else ((isnull(bb.Qty1,0) + isnull(bb.Qty2,0)) * cc.stdtime) end,
  OutFlag = cc.outflag
...
Where cc.OrderNo = @ps_orderno and cc.PartNo = @ps_partno and cc.outflag <> 'P'
