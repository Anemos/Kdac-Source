...
From trouting bb
Where bb.orderno like @ps_orderno
...
From ( Select WcCode = aa.WcCode,
  TotalTime = Sum(convert(decimal(6,1),isnull(aa.stdtime,0) / 60)),
  EndTime =  Sum( Case When aa.workstatus = 'C' Then convert(decimal(6,1),isnull(aa.stdtime,0) / 60) Else 0 End ),
  IngTime =  Sum( Case When aa.workstatus <> 'C' Then convert(decimal(6,1),isnull(aa.stdtime,0) / 60) Else 0 End )
  From trouting aa
