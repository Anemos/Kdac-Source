...
select operno = aa.operno,
  ...
  stdtime = aa.StdTime,
  unitprice = case aa.wccode when 'THT' then dd.heatprice else dd.unitprice end,
  weight = bb.weight,
  ...
  BEGIN
    ...
    if @ls_chkoperno <> @ls_operno
      BEGIN
        ...
        select @li_matcost = @li_weight * @li_unitprice
        select @li_mchcost = @li_matcost + @li_outcostratio * cast( ( @li_stdtime / 60 ) as numeric(6,1) )
        select @li_outcost = @li_mchcost * 1.15

        INSERT INTO TOUTPROCESSDETAIL(OrderNo, PartNo, OutSerial, OperNo,
          StdTime, OutCostRatio, MatCost, MchCost,
          OutCost, LastEmp)
        VALUES (@ps_orderno, @ps_partno, @ls_outserial, @ls_operno,
          @li_stdtime, @li_outcostratio, @li_matcost, @li_mchcost,
          @li_outcost, @ps_empno)
