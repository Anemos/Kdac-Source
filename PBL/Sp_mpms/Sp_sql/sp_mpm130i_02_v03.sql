/*
  ...
  Description : 원가정보(완료) - 외주가공데이타 제외
...
from #order_tmp aa inner join tpartlist bb
  ...
  on bb.orderno = cc.orderno and bb.partno = cc.partno and cc.outflag <> 'P'
...
into #job_tmp1
from #order_tmp aa inner join tpartlist bb
  on bb.orderno = aa.orderno and bb.outflag <> 'P'
...
select ApplyFrom = @ps_applyfrom,
  ...
  FinalQty = 0,
  StdTime  =  cc.StdTime,
  OutCost1 =  case when (aa.OrderNo >= '20070000') then cc.StdOutCost else 0 end,
  MatCost1 =  case when (aa.OrderNo >= '20070000') then (cc.StdMatCost + cc.StdHeatCost) else (cc.StdMatCost + cc.StdHeatCost + cc.StdOutCost) end,
  ManCost1 =  cc.StdManCost,
  MchCost1 =  cc.StdMchCost,
  SumCost1 =  case when (aa.OrderNo >= '20070000') then (cc.StdMatCost + cc.StdManCost + cc.StdMchCost + cc.StdHeatCost) 
                else (cc.StdMatCost + cc.StdManCost + cc.StdMchCost + cc.StdHeatCost + cc.StdOutCost) end,
  TotCost1 =  case when (aa.OrderNo >= '20070000') then convert(decimal(10,0),((cc.StdMatCost + cc.StdManCost + cc.StdMchCost 
                      + cc.StdHeatCost) * 1.2)  + cc.StdOutCost)
                else convert(decimal(10,0),(cc.StdMatCost + cc.StdManCost + cc.StdMchCost 
                      + cc.StdHeatCost + cc.StdOutCost) * 1.2) end,
  TBS1 = cc.TBS,
  TLA1 = cc.TLA,
  TDM1 = cc.TDM,
  TPM1 = cc.TPM,
  ...
  TAM1 = cc.TAM,
  JobTime  =  ee.JobTime,
  OutCost2 =  case when (aa.OrderNo >= '20070000') then ee.JobOutCost else 0 end,
  MatCost2 =  case when (aa.OrderNo >= '20070000') then (ee.JobMatCost + ee.JobHeatCost) 
                else (ee.JobMatCost + ee.JobHeatCost + ee.JobOutCost) end,
  ManCost2 =  ee.JobManCost,
  MchCost2 =  ee.JobMchCost,
  SumCost2 =  case when (aa.OrderNo >= '20070000') then (ee.JobMatCost + ee.JobManCost + ee.JobMchCost + ee.JobHeatCost) 
                else (ee.JobMatCost + ee.JobManCost + ee.JobMchCost + ee.JobHeatCost + ee.JobOutCost) end,
  TotCost2 =  case when (aa.OrderNo >= '20070000') then convert(decimal(10,0),((ee.JobMatCost + ee.JobManCost + ee.JobMchCost
                    + ee.JobHeatCost) * 1.2) + ee.JobOutCost) 
                else convert(decimal(10,0),(ee.JobMatCost + ee.JobManCost + ee.JobMchCost
                    + ee.JobHeatCost + ee.JobOutCost) * 1.2) end,
  TBS2 = dd.TBS,
  TLA2 = dd.TLA,
  TDM2 = dd.TDM,
  TPM2 = dd.TPM,
