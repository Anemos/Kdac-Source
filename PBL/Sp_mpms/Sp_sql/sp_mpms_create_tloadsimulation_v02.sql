...
/*
Execute sp_mpms_create_tloadsimulation 'A'
select * from tloadpriority where versionno = 'A'

update tloadpriority
set allottime = 0, outworkdate = '', loadflag = 'N'
where versionno = 'A'

select * from tloadavailtime where versionno = 'A'
select * from tloadsimulation where versionno = 'A'
*/
...
Execute sp_mpms_create_tloadpriority @ps_versionno
delete from tloadavailtime where versionno = @ps_versionno
delete from tloadsimulation where versionno = @ps_versionno

select @ls_refdate = convert(varchar(10), dateadd(day,1,getdate()), 102)
select @li_check = count(*) from tloadpriority where versionno = @ps_versionno and loadflag = 'N'

while @li_check > 0
  Begin
    select @li_index = 0
    
    exec sp_mpms_create_tloadavailtime @ps_versionno, @ls_refdate
    -- 외주공정 처리
    ...
    while ( @li_index <> 1 and @li_index <> 7 )
      Begin
        ...
        WHERE A.VersionNo = @ps_versionno AND A.OutFlag = 'P' AND 
          A.SerialNo > @li_serialno AND A.LOADFLAG = 'N' AND
        ...
        if (@li_resulttime - (@li_stdtime - @li_allottime)) > 0
          Begin
            ...
            WHERE VersionNo = @ps_versionno AND SerialNo = @li_serialno
            
            INSERT INTO TLOADSIMULATION
            ( VersionNo, PlanDate,SerialNo,OrderNo,PartNo,OperNo,
              WcCode,RemainTime,AllotTime,LoadFlag,LastEmp,LastDate )
            VALUES(@ps_versionno, @ls_refdate,@li_serialno,@ls_orderno,@ls_partno,@ls_operno,
    ...
    while @li_index = 0
      Begin
        ...
        FROM TLOADPRIORITY A
        WHERE A.VersionNo = @ps_versionno AND A.OutFlag <> 'P' AND A.LOADFLAG = 'N' AND 
           A.OUTWORKDATE <> @ls_refdate AND A.SERIALNO > @li_serialno AND
          ( A.PREOPERNO IS NULL OR
          EXISTS ( SELECT * FROM TLOADPRIORITY B
            WHERE A.VersionNo = B.VersionNo AND A.ORDERNO = B.ORDERNO AND A.PARTNO = B.PARTNO AND
        ...
        if @@rowcount = 0
          ...
          Begin
            ...
            from tloadavailtime
            where versionno = @ps_versionno and plandate = @ls_refdate and remaintime <> 0
        ...
        from tloadavailtime
        where versionno = @ps_versionno and plandate = @ls_refdate and wccode = @ls_wccode and 
          remaintime <> 0
        ...
        if (@li_remaintime < (@li_stdtime - @li_allottime))
          Begin
            ...
            SET ApplyTime = ApplyTime + @li_resulttime,
              RemainTime = 0
            WHERE VersionNo = @ps_versionno AND PlanDate = @ls_refdate AND WcCode = @ls_wccode
