...
From trouting bb inner join tpartlist dd
...
From ( select orderno = bb.orderno,
    ...
    operno = bb.operno,
    wccode = bb.wccode,
    stdtime = Sum(convert(decimal(6,1),isnull(bb.StdTime * (isnull(aa.Qty1,0) + isnull(aa.Qty2,0)),0) / 60))
    from trouting bb inner join tpartlist aa
      on bb.orderno = aa.orderno and bb.partno = aa.partno
    where bb.orderno = @ps_orderno and bb.partno like @ps_partno and bb.workstatus = 'C'
