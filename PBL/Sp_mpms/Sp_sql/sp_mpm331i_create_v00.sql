  ...
  BEGIN
    ...
    while @li_index = 0
      BEGIN
        ...
        select @li_totaltime = @li_stdtime - (480 - @li_availtime)
        --- 작업시간 당일 여유시간보다 큰 경우
        If @li_totaltime > 0
          BEGIN
            ...
            while @li_index = 0
              BEGIN
                ...
                If @li_totaltime > 480
                  BEGIN
                    INSERT INTO TLOADPLAN(WorkDate, OrderNo, PartNo, OperNo, WcCode, StdSumTime, StdTime)
                      VALUES(@ls_applydate, @ls_orderno, @ls_partno, @ls_operno, @ls_wccode, @li_stdtime, 480)

                    select @li_totaltime = @li_totaltime - 480
                  END
                Else
                  BEGIN
                    INSERT INTO TLOADPLAN(WorkDate, OrderNo, PartNo, OperNo, WcCode, StdSumTime, StdTime)
                      VALUES(@ls_applydate, @ls_orderno, @ls_partno, @ls_operno, @ls_wccode, @li_stdtime, @li_totaltime)
                    BREAK
                  END
              END
        ...
        Else
          BEGIN
            INSERT INTO TLOADPLAN(WorkDate, OrderNo, PartNo, OperNo, WcCode, StdSumTime, StdTime)
              VALUES(@ls_applydate, @ls_orderno, @ls_partno, @ls_operno, @ls_wccode, @li_stdtime, @li_stdtime)
            BREAK
          END
      END
    --- 01 END
