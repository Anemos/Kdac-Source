/*
  File Name : sp_mpm130i_02.SQL
  SYSTEM    : MPMS System
  Procedure Name  : sp_mpm130i_02
  Description : 원가정보(완료)
  Use DataBase  : MPMS
  Use Program :
...
If Exists (Select * From sysobjects
      Where id = object_id(N'[dbo].[sp_mpm130i_02]')
        And OBJECTPROPERTY(id, N'IsProcedure') = 1)
  Drop Procedure [dbo].[sp_mpm130i_02]
GO

/*
Execute sp_mpm130i_02 '2004.04.01','2004.05.06','%','%'
*/

Create Procedure sp_mpm130i_02
...
select OrderNo = bb.OrderNo,
  ...
  StdTime = Sum(convert(decimal(6,1),isnull(cc.StdTime,0) / 60 )),
  StdMchCost = Sum(0),
  StdManCost = Sum(convert(decimal(10,0),(isnull(cc.StdTime,0) / 60) * isnull(aa.laborcost,0))),
  StdHeatCost = Sum(isnull(cc.StdHeatCost,0)),
  StdOutCost = Sum(isnull(cc.StdOutCost,0)),
  TBS = Sum(case cc.WcCode when 'TBS' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TLA = Sum(case cc.WcCode when 'TLA' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TDM = Sum(case cc.WcCode when 'TDM' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TPM = Sum(case cc.WcCode when 'TPM' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TBM = Sum(case cc.WcCode when 'TBM' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TMM = Sum(case cc.WcCode when 'TMM' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TJB = Sum(case cc.WcCode when 'TJB' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  THT = Sum(case cc.WcCode when 'THT' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TUG = Sum(case cc.WcCode when 'TUG' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TSG = Sum(case cc.WcCode when 'TSG' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TJG = Sum(case cc.WcCode when 'TJG' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TVG = Sum(case cc.WcCode when 'TVG' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TWC = Sum(case cc.WcCode when 'TWC' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TED = Sum(case cc.WcCode when 'TED' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TLP = Sum(case cc.WcCode when 'TLP' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end),
  TAM = Sum(case cc.WcCode when 'TAM' then convert(decimal(6,1),isnull(cc.StdTime,0) / 60 ) else 0 end)
into #std_tmp1
...
from #order_tmp aa inner join tmatcost bb
  on aa.orderno = bb.orderno
where bb.yearmm >= convert(char(6),@ld_fromdt,112) and 
  bb.yearmm <= convert(char(6),@ld_todt,112)
group by bb.orderno, bb.partno
