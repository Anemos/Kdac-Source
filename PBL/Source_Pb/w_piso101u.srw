...
If ls_messagegubun = "" Or IsNull(ls_messagegubun) = True Or Len(ls_messagegubun) = 0 Then
	MessageBox("메세지전송 관리", "정확한 메세지 구분 코드를 등록하십시오.", StopSign!)
	Return
End If

If ls_pcip = "" Or IsNull(ls_pcip) = True Or Len(ls_pcip) = 0 Then
	MessageBox("메세지전송 관리", "정확한 IP Address 코드를 등록하십시오.", StopSign!)
	Return
End If
...
If ll_count > 0 Then
	MessageBox("메세지전송 관리", "등록한 메세지구분 및 IP Address 는 이미 등록되어 있습니다." + &
									"~r~n~r~n새로운 메세지구분 및 IP Address 를 등록하십시오.", StopSign!)
	Return
End If

If MessageBox("메세지전송 관리", "새로운 메세지전송 관리 정보를 저장하시겠습니까?", &
...
If SQLPIS.sqlcode = 0 Then
	...
	ib_save	= True
	MessageBox("메세지전송 관리", "새로운 메세지전송 관리 정보를 저장하였습니다.", Information!)
	wf_retrieve()
...
Else
	...
	SQLPIS.AutoCommit = True
	MessageBox("메세지전송 관리", "새로운 메세지전송 관리 정보를 저장하는 중에 오류가 발생하였습니다.", StopSign!)
End If
