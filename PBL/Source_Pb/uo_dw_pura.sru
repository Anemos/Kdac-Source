$PBExportHeader$uo_dw_pura.sru
$PBExportComments$데이타 윈도우 표준
forward
global type uo_dw_pura from datawindow
end type
end forward
...
on ue_accepttext;//item이 수정된후 아직 인정이 안된상태로 남아있을 경우에 대비해서
...
on losefocus;//item이 수정된후 아직 인정이 안된상태로 남아있을 경우에 대비해서
//focus가 dw를 떠날때 수정된 내용을 item의 data로 받아들임
...
on uo_dw_pura.create
end on

on uo_dw_pura.destroy
end on
