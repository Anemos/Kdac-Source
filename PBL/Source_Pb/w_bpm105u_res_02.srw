$PBExportHeader$w_bpm105u_res_02.srw
$PBExportComments$상위품번 복사 Response Window
forward
global type w_bpm105u_res_02 from window
end type
type p_1 from picture within w_bpm105u_res_02
end type
type dw_1 from datawindow within w_bpm105u_res_02
end type
type sle_1 from singlelineedit within w_bpm105u_res_02
end type
type sle_2 from singlelineedit within w_bpm105u_res_02
end type
type cb_1 from commandbutton within w_bpm105u_res_02
end type
type cb_2 from commandbutton within w_bpm105u_res_02
end type
type gb_1 from groupbox within w_bpm105u_res_02
end type
type gb_2 from groupbox within w_bpm105u_res_02
end type
end forward
...
global w_bpm105u_res_02 w_bpm105u_res_02
...
//	end if
		select count(*) into : l_n_cnt from "PBBPM"."BPM001"
			where "PBBPM"."BPM001"."PCMCD" = '01' and
					...
					"PBBPM"."BPM001"."PCITN" = :l_s_pcitn AND  
		...
		if l_n_cnt > 0 then
			...
			for i = 1 to l_n_rows
				...
				if l_s_pcitn = l_s_itno then
					...
					if f_chk_bom_bpm(l_s_plant,l_s_div,l_s_itno,l_s_pcitn) = 1 then	
						messagebox("확인","BOM 구조를 확인하세요.")
						return
					end if
				end if
...
for i = 1 to l_n_rows 
	...
	l_s_itclsb = f_bom_get_itcls(l_s_plant,l_s_div,dw_1.object.ppitn[i])
   l_s_srce   = f_bom_get_srce(l_s_plant,l_s_div,dw_1.object.ppitn[i])
