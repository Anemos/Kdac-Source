$PBExportHeader$u_pism_date_wday.sru
$PBExportComments$일자 선택 - 금일 - is_uo_date
forward
global type u_pism_date_wday from userobject
end type
type st_name from statictext within u_pism_date_wday
end type
type pb_next from picturebutton within u_pism_date_wday
end type
type pb_prev from picturebutton within u_pism_date_wday
end type
type sle_date from singlelineedit within u_pism_date_wday
end type
type pb_ddlb from picturebutton within u_pism_date_wday
end type
type dw_cal from datawindow within u_pism_date_wday
end type
end forward
...
global u_pism_date_wday u_pism_date_wday
...
public subroutine uf_set_date (string as_newdate)
end prototypes
...
event ue_select;str_pism_daily lstr_mh 
String ls_wGbn 

lstr_mh = Dynamic GetParent().wf_getistr()

If IsValid(lstr_mh) Then 
	ls_wGbn = f_pism_getdayworkgbn(lstr_mh.area, lstr_mh.div, lstr_mh.wc, is_uo_date)
	If ls_wGbn = 'H' Then 
		sle_date.BackColor = RGB(255, 200, 200)
	Else
		sle_date.BackColor = RGB(255, 255, 255)
	End If 
End If 
end event
...
public subroutine uf_set_date (string as_newdate);sle_date.Text = as_newdate 

id_uo_date = Date(as_newdate) 
...
event constructor;width  = 759
height = 73

BringToTop = True
id_uo_date = RelativeDate(Date(f_pisc_get_date_applydate("%", "%", f_pisc_get_date_nowtime())), -1)
is_uo_date = String(id_uo_date, 'YYYY.MM.DD')
init_cal(id_uo_date)
set_date_format ('yyyy.mm.dd')
TriggerEvent("ue_variable_set")
...
string picturename = "bmp\arrow_next.bmp"
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string picturename = "bmp\arrow_prior.bmp"
alignment htextalign = left!
end type
...
Else
//	gb_focus = True
//	MessageBox("일자 오류", "잘못된 일장 입니다.~n~r정확한 일자를 입력하십시요.")
//	init_cal(today())
//	id_uo_date = today()
//	is_uo_date = String(id_uo_date, 'YYYY.MM.DD')
//	set_date_format ( 'yyyy.mm.dd' )
...
string picturename = "bmp\arrow_ddlb.bmp"
alignment htextalign = left!
end type
...
integer height = 572
integer taborder = 10
string dataobject = "d_pisc_dddw_cal"
