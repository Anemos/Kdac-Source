$PBExportHeader$w_pisr081b.srw
$PBExportComments$간판단위가입고취소처리(정상)
forward
global type w_pisr081b from w_ipis_sheet01
end type
type uo_area from u_pisc_select_area within w_pisr081b
end type
type uo_division from u_pisc_select_division within w_pisr081b
end type
type dw_pisr080b_02 from u_vi_std_datawindow within w_pisr081b
end type
type dw_scan from datawindow within w_pisr081b
end type
type dw_summary from datawindow within w_pisr081b
end type
type gb_3 from groupbox within w_pisr081b
end type
type gb_scan from groupbox within w_pisr081b
end type
type dw_pisr080b_01 from datawindow within w_pisr081b
end type
end forward
...
CHOOSE CASE ls_judgeFlag
	CASE '1'
//			li_Net = MessageBox("간판확인요청", "합격 처리된 간판입니다. ~r~n~r~n" &
//				+ "해당 간판을 가입고취소처리 하시겠습니까 ?", &
//										Question!, OKCancel!, 1)
//			if li_Net <> 1 then
//				return -1
//			end if
			uo_status.st_message.text = "합격 처리된 간판입니다. 가입고취소 확인! "
//	CASE '2'
...
type uo_status from w_ipis_sheet01`uo_status within w_pisr081b
end type
...
//		가입고취소 간판 저장
////////////////////////////////////////////////////////////
String	ls_suppCode, ls_itemCode, ls_deliveryno, ls_areacode, ls_divcode, ls_mark
String	ls_receiptdate
...
// 검사성적서 가입고처리내역 업데이트
ls_areaCode			= This.GetItemString( ll_Row, 'areacode' )
ls_divCode			= This.GetItemString( ll_Row, 'divisioncode' )
ls_suppCode			= This.GetItemString( ll_Row, 'suppliercode' )
ls_itemCode			= This.GetItemString( ll_Row, 'itemcode' )
...
UPDATE TQQCRESULT
SET KDACREMARK = KDACREMARK + :ls_mark
WHERE AREACODE = :ls_areacode AND DIVISIONCODE = :ls_divcode AND
		SUPPLIERCODE = :ls_suppcode AND ITEMCODE = :ls_itemcode AND
