$PBExportHeader$w_pisp050i.srw
$PBExportComments$일일 생산계획(모델군별)
forward
global type w_pisp050i from w_ipis_sheet01
end type
type dw_1 from u_vi_std_datawindow within w_pisp050i
end type
type uo_area from u_pisc_select_area within w_pisp050i
end type
type uo_division from u_pisc_select_division within w_pisp050i
end type
type uo_productgroup from u_pisc_select_productgroup within w_pisp050i
end type
type uo_modelgroup from u_pisc_select_modelgroup within w_pisp050i
end type
type uo_date from u_pisc_date_applydate within w_pisp050i
end type
type dw_print from datawindow within w_pisp050i
end type
type gb_1 from groupbox within w_pisp050i
end type
type gb_2 from groupbox within w_pisp050i
end type
type gb_3 from groupbox within w_pisp050i
end type
end forward
...
If dw_1.Retrieve(ls_todate, &
	...
	uo_status.st_message.text =  "일일생산계획(모델군별) 정보" //+ f_message("변경된 데이타가 없습니다.")
Else
	uo_status.st_message.text =  "일일생산계획(모델군별)이 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
	MessageBox("일일생산계획(모델군별)", "일일생산계획(모델군별)이 존재하지 않습니다")
End If
end event
...
lstr_prt.title			= '일일생산계획(모델군별)'
lstr_prt.tag			= '일일생산계획(모델군별)'
...
type uo_status from w_ipis_sheet01`uo_status within w_pisp050i
end type
...
string dataobject = "d_pisp050i_01"
...
string title = "인쇄"
string dataobject = "d_pisp050i_01_print"
boolean hscrollbar = true
boolean vscrollbar = true
