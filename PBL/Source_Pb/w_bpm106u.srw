$PBExportHeader$w_bpm106u.srw
$PBExportComments$Option 품목관리용윈도우
forward
global type w_bpm106u from w_origin_sheet02
end type
type dw_mainitem from datawindow within w_bpm106u
end type
type st_mdetail from statictext within w_bpm106u
end type
type dw_regitem from datawindow within w_bpm106u
end type
type rb_1 from radiobutton within w_bpm106u
end type
type rb_2 from radiobutton within w_bpm106u
end type
type pb_fitno from picturebutton within w_bpm106u
end type
type st_sdetail from statictext within w_bpm106u
end type
type gb_1 from groupbox within w_bpm106u
end type
type dw_subitem from datawindow within w_bpm106u
end type
type uo_1 from uo_plandiv_pdcd within w_bpm106u
end type
end forward
...
public function integer wf_get_chkitem (string a_plant, string a_div, string a_item);// Check main or subitem in DEPDMO.BPM001

string ls_check
  SELECT "PBBPM"."BPM001"."PCITN"  
  		into :ls_check
  FROM "PBBPM"."BPM001"  
  WHERE "PBBPM"."BPM001"."PLANT" = :a_plant AND
  			"PBBPM"."BPM001"."PCITN" = :a_item AND
  		  "PBBPM"."BPM001"."PDVSN" = :a_div  using sqlca;
...
dw_mainitem.retrieve(l_s_plant,l_s_div,l_s_pdcd,g_s_date) 
...
//부품번 등록초기화
if ll_currow > 0 then
	ls_mainitem = trim(dw_mainitem.object.BPM003_opitn[ll_currow])
...
//	end if	
	...
	ls_rtnchk = f_bom_check_bpm(ls_plant,ls_div,ls_chkmain,ls_chksub,ls_chkdtm,ls_chkdte) 
	if f_spacechk(ls_rtnchk) = -1 then
		//nothing
	elseif mid(ls_rtnchk,1,1) = "i" then
		uo_status.st_message.text=f_message("E330")
		openwithparm(w_bPm106u_detail,(ls_plant + ls_div + ls_chkmain))
		return 0
...
if i_s_selected = "i" then
	...
	SELECT count(*) INTO :li_mcnt
		FROM "PBBPM"."BPM003"
		WHERE "PBBPM"."BPM003"."OPLANT"= :ls_plant AND
				"PBBPM"."BPM003"."ODVSN"= :ls_div AND
				"PBBPM"."BPM003"."OPITN"= :ls_chkmain AND
		 		"PBBPM"."BPM003"."OFITN"= :ls_chksub AND
				("PBBPM"."BPM003"."OEDTE" >= :g_s_date OR
				"PBBPM"."BPM003"."OEDTE" = '')  using sqlca; 
	...
	SELECT count(*) INTO :li_mcnt
		...
		WHERE "PBBPM"."BPM003"."OPLANT"= :ls_plant AND
				"PBBPM"."BPM003"."ODVSN"= :ls_div AND
				"PBBPM"."BPM003"."OPITN"= :ls_chksub AND
		 		"PBBPM"."BPM003"."OFITN"= :ls_chkmain AND
				("PBBPM"."BPM003"."OEDTE" >= :g_s_date OR
				"PBBPM"."BPM003"."OEDTE" = '       ')  using sqlca; 
...
if i_s_selected="i" then
	...
	SELECT count(*) INTO :li_scnt										
		...
		WHERE "PBBPM"."BPM003"."OPLANT"= :ls_plant AND
				...
				"PBBPM"."BPM003"."OPITN"= :ls_chkmain AND
				"PBBPM"."BPM003"."OFITN"= :ls_chksub AND
				"PBBPM"."BPM003"."OCHDT"= :g_s_date  using sqlca;
...
//				SET "OEDTE" = :ls_oedtm_hold 
         DELETE FROM "PBBPM"."BPM003"
				WHERE "PBBPM"."BPM003"."OPLANT" = :ls_plant AND
						"PBBPM"."BPM003"."ODVSN" = :ls_div AND
						"PBBPM"."BPM003"."OPITN"= :ls_chkmain  using sqlca;
		...
		if rb_1.checked = false then
			...
			UPDATE "PBBPM"."BPM003"
				...
				WHERE "PBBPM"."BPM003"."OPLANT" = :ls_plant AND
						...
						"PBBPM"."BPM003"."OPITN"= :ls_chkmain AND 
						"PBBPM"."BPM003"."OFITN" = :ls_chksub AND
						"PBBPM"."BPM003"."OCHDT"= :ls_ochdt using sqlca;
...
ls_mainitem = trim(dw_mainitem.object.BPM003_opitn[ll_curmain])
...
//	SET "OEDTE"= :g_s_date, "OCHCD" = 'D'
   DELETE FROM "PBBPM"."BPM003"
	WHERE "PBBPM"."BPM003"."OPLANT" = :ls_plant AND
			"PBBPM"."BPM003"."ODVSN" = :ls_div AND
			"PBBPM"."BPM003"."OPITN"= :ls_mainitem AND 
			"PBBPM"."BPM003"."OFITN" = :ls_subitem
//			AND
//			"PBBPM"."BPM003"."OCHDT"= :ls_ochdt 
			using sqlca;
	if sqlca.sqlcode = 0 then
		commit using sqlca;
	else
...
ids_u_date.dataobject = "d_bpm001_106u_redate"
...
type uo_status from w_origin_sheet02`uo_status within w_bpm106u
end type
...
string dataobject = "d_bpm001_priitem"
...
if ll_currow < 1 then
	...
	ls_opitn=dw_mainitem.object.BPM003_opitn[ll_currow]
...
ls_data = dw_mainitem.object.BPM003_opitn[row]
...
integer taborder = 40
boolean bringtotop = true
string dataobject = "d_bpm001_regitem"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
integer height = 1544
integer taborder = 30
string dataobject = "d_bpm001_secitem"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
