$PBExportHeader$w_pisk260i.srw
$PBExportComments$월별생산추이(모델군별)
forward
global type w_pisk260i from w_ipis_sheet01
end type
type dw_1 from u_vi_std_datawindow within w_pisk260i
end type
type uo_area from u_pisc_select_area within w_pisk260i
end type
type uo_division from u_pisc_select_division within w_pisk260i
end type
type dw_detail from datawindow within w_pisk260i
end type
type st_h_bar from uo_xc_splitbar within w_pisk260i
end type
type gb_1 from groupbox within w_pisk260i
end type
type gb_2 from groupbox within w_pisk260i
end type
type uo_modelgroup from u_pisc_select_modelgroup within w_pisk260i
end type
type uo_productgroup from u_pisc_select_productgroup within w_pisk260i
end type
type uo_year from u_pisc_date_scroll_year within w_pisk260i
end type
type dw_print from datawindow within w_pisk260i
end type
type gb_3 from groupbox within w_pisk260i
end type
end forward
...
global w_pisk260i w_pisk260i

type variables
Boolean	ib_open
end variables

on w_pisk260i.create
int iCurrent
call super::create
...
this.Control[iCurrent+9]=this.uo_productgroup
this.Control[iCurrent+10]=this.uo_year
this.Control[iCurrent+11]=this.dw_print
this.Control[iCurrent+12]=this.gb_3
end on

on w_pisk260i.destroy
call super::destroy
destroy(this.dw_1)
...
If dw_1.Retrieve(	uo_year.is_uo_year, &
						...
						'%') > 0 Then
	uo_status.st_message.text =  "월별 생산추이 정보" //+ f_message("변경된 데이타가 없습니다.")
Else
	uo_status.st_message.text =  "월별 생산추이 정보가 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
	MessageBox("월별 생산추이(모델군별)", "월별 생산추이 정보가 존재하지 않습니다")
End If
...
lstr_prt.title			= '월별생산추이(모델군별)'
lstr_prt.tag			= '월별생산추이(모델군별)'
lstr_prt.dwsyntax		= ls_mod
OpenSheetWithParm(w_prt, lstr_prt, w_frame, 0, Layered!)

end event

type uo_status from w_ipis_sheet01`uo_status within w_pisk260i
end type
...
string dataobject = "d_pisk260i_01"
...
event rowfocuschanged;String	ls_prdyear, ls_areacode, ls_divisioncode, ls_productgroup, ls_modelgroup, &
			ls_itemcode, ls_series_name

If GetRow() > 0 Then
	SelectRow(0, False)
	SelectRow(CurrentRow, True)
	ls_prdyear			= Trim(GetItemString(CurrentRow, "PrdYear"))
	...
	dw_detail.ReSet()
	If dw_detail.Retrieve(ls_prdyear, &
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_pisk260i_02"
boolean hscrollbar = true
boolean vscrollbar = true
...
string title = "none"
string dataobject = "d_pisk260i_01_print"
boolean hscrollbar = true
boolean vscrollbar = true
