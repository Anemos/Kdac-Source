$PBExportHeader$w_piss471u.srw
$PBExportComments$출고전표수정
forward
global type w_piss471u from w_ipis_sheet01
end type
type dw_sheet from u_vi_std_datawindow within w_piss471u
end type
type uo_area from u_pisc_select_area within w_piss471u
end type
type uo_division from u_pisc_select_division within w_piss471u
end type
type uo_date from u_pisc_date_applydate within w_piss471u
end type
type dw_save from datawindow within w_piss471u
end type
type dw_kbno_create from datawindow within w_piss471u
end type
type dw_print from datawindow within w_piss471u
end type
type dw_3 from datawindow within w_piss471u
end type
type gb_1 from groupbox within w_piss471u
end type
end forward
...
if Sqlpis.sqlcode = 0 then
	...
	uo_status.st_message.text = "tlostno error "
	return false
End if
end function
...
for i = 1 to ll_rowcount step 1
 ...
 next
next
uo_status.st_message.text = '간판 수정완료(wf_kbno_create)'
return lb_commit
end function
...
if isnull(ln_rowcount) or ln_rowcount = 0 then
		...
		values	( :is_shipdate,:ls_areacode,:ls_divisioncode,:ls_itemcode,:ls_partid,0,0,:fl_shipqty * (-1),:g_s_empno,getdate())
		using	sqlpis ;
		if sqlpis.sqlcode <> 0 then
			return false
		end if
...
long ll_child_qty,ll_parent_qty,ll_count
ls_close_date = f_pisc_get_date_applydate_close("%", "%",f_pisc_get_date_nowtime())
...
//		ls_shipsheetno  = dw_sheet.GetItemString(ll_i, 'shipsheetno')
      ...
      ls_item_type_check = f_piss_itemtype_check(is_areacode,ls_divisioncode,ls_itemcode)
		...
		if ls_shipgubun <> 'M' then //이체는 인터페이스암함 
	   ...
	   if not wf_update_tshipsheet(is_areacode,ls_divisioncode,ls_shipsheetno,ls_srno,ll_changeqty) then
		...
		if ls_shipgubun = 'D' OR ls_shipgubun = 'G' OR ls_shipgubun = 'H' OR ls_shipgubun = 'K' OR &
	   	ls_shipgubun = 'R' OR ls_shipgubun = 'U' OR ls_shipgubun = 'V' OR ls_shipgubun = 'W' THEN // A/S,KD 일경우만..
			if not wf_update_tseqinv(ls_custcode,ls_custitemcode,ll_changeqty) then
			 uo_status.st_message.text = "tseqinv update error"
				lb_commit = false
				exit;
			else
				lb_commit = true
...
type uo_status from w_ipis_sheet01`uo_status within w_piss471u
end type
