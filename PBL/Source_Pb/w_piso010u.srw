...
If dw_1.Retrieve(	uo_area.is_uo_areacode) > 0 Then
	uo_status.st_message.text =  "지역마스터 정보" //+ f_message("변경된 데이타가 없습니다.")
Else
	uo_status.st_message.text =  "지역마스터 정보가 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
	MessageBox("지역마스터", "지역마스터 정보가 존재하지 않습니다")
End If
