$PBExportHeader$w_pisk110i.srw
$PBExportComments$간판준수율 - 일별
forward
global type w_pisk110i from w_ipis_sheet01
end type
type dw_1 from u_vi_std_datawindow within w_pisk110i
end type
type uo_area from u_pisc_select_area within w_pisk110i
end type
type uo_division from u_pisc_select_division within w_pisk110i
end type
type uo_workcenter from u_pisc_select_workcenter within w_pisk110i
end type
type uo_line from u_pisc_select_line within w_pisk110i
end type
type dw_detail from datawindow within w_pisk110i
end type
type st_h_bar from uo_xc_splitbar within w_pisk110i
end type
type uo_month from u_pisc_date_scroll_month within w_pisk110i
end type
type dw_data from datawindow within w_pisk110i
end type
type dw_print from datawindow within w_pisk110i
end type
type gb_1 from groupbox within w_pisk110i
end type
type gb_2 from groupbox within w_pisk110i
end type
type gb_3 from groupbox within w_pisk110i
end type
end forward
...
global w_pisk110i w_pisk110i

type variables
Boolean	ib_open
end variables

on w_pisk110i.create
int iCurrent
call super::create
...
//	uo_status.st_message.text =  "일별 간판 준수율 정보" //+ f_message("변경된 데이타가 없습니다.")
//Else
//	uo_status.st_message.text =  "일별 간판 준수율 정보가 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
//	MessageBox("일별 간판 준수율", "일별 간판 준수율 정보가 존재하지 않습니다")
//End If
...
Long		i, j, ll_rowcount, &
			ll_release_all, ll_prd_all, ll_different_all, &
			ll_release[31], ll_prd[31], ll_different[31], &
			ll_release_all_sub, ll_prd_all_sub, ll_different_all_sub, &
			ll_release_sub[31], ll_prd_sub[31], ll_different_sub[31], &
			ll_release_all_total, ll_prd_all_total, ll_different_all_total, &
			ll_release_total[31], ll_prd_total[31], ll_different_total[31]
Decimal	ld_prdrate_all, ld_prdrate[31], &
			ld_prdrate_all_sub, ld_prdrate_sub[31], &
			ld_prdrate_all_total, ld_prdrate_total[31]

iw_this.TriggerEvent("ue_reset")

ll_rowcount = dw_data.Retrieve(	uo_month.is_uo_month, &
						uo_area.is_uo_areacode,				uo_division.is_uo_divisioncode, &
...
//						ld_prdrate_sub[j]	= Round((((ll_prd_sub[j] - ll_different_sub[j]) * 1.0) / ll_release_sub[j]) * 100.0, 2)
			...
			ll_prd_all_sub			= 0
			ll_different_all_sub	= 0
			ld_prdrate_all_sub	= 0
			For j = 1 To 31
				ll_release_sub[j]		= 0
				ll_prd_sub[j]			= 0
				ll_different_sub[j]	= 0
				ld_prdrate_sub[j]		= 0
			Next
...
// 공장별 구하기
	...
	ll_prd_all			= dw_data.GetItemNumber(i, "PrdCount")
	ll_different_all	= dw_data.GetItemNumber(i, "DifferentCount")	
	For j = 1 To 31
		ll_release[j]		= dw_data.GetItemNumber(i, "ReleaseCount" + Right('0' + String(j), 2))
		ll_prd[j]			= dw_data.GetItemNumber(i, "PrdCount" + Right('0' + String(j), 2))
		ll_different[j]	= dw_data.GetItemNumber(i, "DifferentCount" + Right('0' + String(j), 2))
	Next
...
event ue_reset;call super::ue_reset;dw_data.ReSet()
dw_1.ReSet()
dw_detail.ReSet()
dw_print.ReSet()
end event
...
lstr_prt.title			= '일별 간판 준수율'
lstr_prt.tag			= '일별 간판 준수율'
lstr_prt.dwsyntax		= ls_mod
OpenSheetWithParm(w_prt, lstr_prt, w_frame, 0, Layered!)

end event

type uo_status from w_ipis_sheet01`uo_status within w_pisk110i
end type
...
//
//	dw_detail.Retrieve(	uo_month.is_uo_month, &
...
string title = "none"
string dataobject = "d_pisk110i_01"
boolean hscrollbar = true
boolean vscrollbar = true
...
boolean titlebar = true
string title = "none"
string dataobject = "d_pisk_date_print"
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
boolean hsplitscroll = true
