...
// Invoice No가 입력된 경우에 해당 데이타를 조회하여 선택하도록 한다.
//----------------------------
...
if f_spacechk(ls_invoiceno) = -1 then
	if MessageBox("확인", "InvoceNo가 입력되지 않았습니다. 라벨을 추가하시겠습니까?", &
...
if li_selrow < 1 then
	...
	if ls_customercode = '%' or ls_labelgubun = '%' then
		uo_status.st_message.text = "고객사코드 와 라벨구분이 선택되지 않았습니다."
		return 0
	end if
	choose case ls_labelgubun
		case 'DH'
		...
		case else
			uo_status.st_message.text = "정의된 라벨이 아닙니다. 시스템개발팀 담당자에게 연락바랍니다."
			return 0
	end choose
else
...
if f_spacechk(ls_invoiceno) <> -1 then
	...
	if lstr_parm.integer_arg[1] = -1 then
		uo_status.st_message.text = "InvoiceNo에 해당하는 SR이 존제하지 않습니다."
		return 0
	end if
else
	if li_selrow > 0 then
