...
String	ls_datetime,	ls_EndDateTime, ls_startdate, ls_enddate, ls_applydate, ls_closedate, ls_lastmonth
...
CHOOSE CASE is_wip_id
	...
	CASE '090'
		...
		// 한달전 데이타 자동삭제처리
		ls_lastmonth = f_relativedate(ls_applydate,-60)
		
		DELETE FROM PBWIP.WIP002D
		WHERE WBCMCD = '01' AND WBYYMMDD < :ls_lastmonth
		using sqlca;
		...
		DELETE FROM PBWIP.WIP003D
		WHERE WCCMCD = '01' AND WCYYMMDD < :ls_lastmonth
		using sqlca;
END CHOOSE
