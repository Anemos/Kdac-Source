...
$PBExportComments$비간판 판정결과 취소 관리화면(운영자용)
forward
...
IF dw_pisq910i.GetSelectedRow(0) = 0 THEN
	MessageBox('확 인', '대상이 없습니다', StopSign!)
	RETURN
END IF
...
IF Trim(ls_chargename) = '델파이' THEN
	// 작성자명이 '델파이'인것은 성적서 없이 자동 작성된것이므로 성적서를 삭제한다
	//
	...
	END IF
ELSE
	// 작성자명이 '델파이'가 아닌것은 업체에서 작성한 성적서이므로 초기화 처리한다
	//
	...
	IF SQLPIS.SQLCode = 0 THEN
		COMMIT USING SQLPIS;
		SQLPIS.AUTOCommit = TRUE
		MessageBox('확 인', '성적서가 초기화 처리 되었습니다')
	ELSE
		ROLLBACK USING SQLPIS;
		SQLPIS.AUTOCommit = TRUE
		MessageBox('확 인', '성적서 초기화 처리에 실패했습니다.')
	END IF
END IF
