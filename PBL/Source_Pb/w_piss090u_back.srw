$PBExportHeader$w_piss090u_back.srw
$PBExportComments$상차계획
forward
global type w_piss090u_back from w_ipis_sheet01
end type
type uo_date from u_pisc_date_applydate within w_piss090u_back
end type
type uo_area from u_pisc_select_area within w_piss090u_back
end type
type uo_division from u_pisc_select_division within w_piss090u_back
end type
type cbx_1 from checkbox within w_piss090u_back
end type
type cb_srview from commandbutton within w_piss090u_back
end type
type cb_split from commandbutton within w_piss090u_back
end type
type cb_print from commandbutton within w_piss090u_back
end type
type tab_1 from tab within w_piss090u_back
end type
...
type dw_truck from datawindow within w_piss090u_back
end type
type cb_trucknew from commandbutton within w_piss090u_back
end type
type dw_plancancel from datawindow within w_piss090u_back
end type
type st_2 from statictext within w_piss090u_back
end type
type st_3 from statictext within w_piss090u_back
end type
type st_4 from statictext within w_piss090u_back
end type
type st_shipqty from statictext within w_piss090u_back
end type
type st_minapqty from statictext within w_piss090u_back
end type
type st_invqty from statictext within w_piss090u_back
end type
type dw_id from datawindow within w_piss090u_back
end type
type dw_truckorder from datawindow within w_piss090u_back
end type
type dw_print from datawindow within w_piss090u_back
end type
type sle_checksrno from singlelineedit within w_piss090u_back
end type
type em_shipeditno from editmask within w_piss090u_back
end type
type sle_custcode from singlelineedit within w_piss090u_back
end type
type st_5 from statictext within w_piss090u_back
end type
type uo_shipoemgubun from u_pisc_select_code within w_piss090u_back
end type
type gb_1 from groupbox within w_piss090u_back
end type
type gb_2 from groupbox within w_piss090u_back
end type
type gb_truck from groupbox within w_piss090u_back
end type
type gb_4 from groupbox within w_piss090u_back
end type
type gb_5 from groupbox within w_piss090u_back
end type
type gb_6 from groupbox within w_piss090u_back
end type
type gb_8 from groupbox within w_piss090u_back
end type
type gb_7 from groupbox within w_piss090u_back
end type
type gb_9 from groupbox within w_piss090u_back
end type
type gb_10 from groupbox within w_piss090u_back
end type
end forward
...
st_3 st_3
st_4 st_4
st_shipqty st_shipqty
st_minapqty st_minapqty
st_invqty st_invqty
dw_id dw_id
...
global w_piss090u_back w_piss090u_back
...
public function boolean wf_update_tloadplan_child (string fs_shipplandate, long fl_truckorder, string fs_srno, string fs_areacode, string fs_divisioncode, long fl_truckplan)
end prototypes
...
//	if (li_truckorder <> fi_truckorder) or (li_truckorder = 0) then
	if li_truckorder = 0 then		
		...
		li_remainqty1 	= adw_source.getitemNumber(ll_selected_rows[ll_i], "shipRemainQty1")
		li_loadqty	 	= adw_source.getitemNumber(ll_selected_rows[ll_i], "truckLoadQty")	
		li_dansuqty	 	= adw_source.getitemNumber(ll_selected_rows[ll_i], "trcukDansuQty")							
		li_rackqty	 	= adw_source.getitemNumber(ll_selected_rows[ll_i], "RackQty")				
...
public subroutine wf_filter ();setpointer(hourglass!)
...
Select Sum(Invqty + repairqty)
  Into :li_invQty
  From tinv
 Where Itemcode	Like :is_modelid
...
public function boolean wf_update_loadplan ();long   	ll_truckorder, ll_loadqty, ll_dansuqty,ll_truckplan
...
If sqlpis.sqlcode = 0 Then
		...
		For i = 1 To ll_rowcount
         ...
         ls_kitgubun   = idw_normal.GetItemString(i,'kitgubun')
			...
			VALUES ( :is_date,:is_areacode,:ls_divisioncode,:ls_srno,:ll_truckorder,:ls_itemcode,:ls_custcode,:ll_shipeditno,
					  :is_date,Null,:ll_truckplan,0,'N','Y',GetDate())
					  using sqlpis;
			...
			If sqlpis.sqlcode <> 0 Then
				messagebox("",'tlaodplanhis 정상 error : ' +sqlpis.sqlerrtext)
				lb_commit = False
				Return lb_commit
			End if			
			if ls_kitgubun = 'Y' then //KIT제품은 자품번도 tloadplan에 저장
			   if not wf_update_tloadplan_child(is_date,ll_truckorder,ls_srno,is_areacode,ls_divisioncode,ll_truckplan) then
	...
	If ll_rowcount > 0 Then
		For i = 1 To ll_rowcount
			...
			If sqlpis.sqlcode <> 0 Then
				messagebox("오류","tloadplan error :" + sqlpis.sqlerrtext)
				lb_commit = False
				Return lb_commit
			End if			
			INSERT INTO TLOADPLANHIS  
            ...
            VALUES ( :is_date,:is_areacode,:ls_divisioncode,:ls_srno,:ll_truckorder,null,null,:ls_shipgubun,   
                     :ls_itemcode,:ls_custcode,:ll_shipeditno,:is_date,:ll_truckplan,0,'N','N','N',null,
                     'Y',getdate()) using sqlpis;

			If sqlpis.sqlcode <> 0 Then
				messagebox("",'tlaodplanhis error : ' +sqlpis.sqlerrtext)
				lb_commit = False
				Return lb_commit
			End if			
			if ls_kitgubun = 'Y' then //KIT제품은 자품번도 tloadplan에 저장
...
public function boolean wf_update_tloadplan_child (string fs_shipplandate, long fl_truckorder, string fs_srno, string fs_areacode, string fs_divisioncode, long fl_truckplan);long ll_parent_qty,ll_child_qty,ll_count,ll_shipeditno
...
DO WHILE sqlpis.sqlcode <> 100 
   ...
   VALUES (:fs_shipplandate,   
           ...
           getdate() )
			  using sqlpis;
	if sqlpis.sqlcode <> 0 then
		messagebox("확인","tloadplan child error : " + sqlpis.sqlerrtext)
		lb_commit = false
		exit;
   else
		lb_commit = true
   ...
   VALUES ( :fs_shipplandate,   
           ...
           getdate() )
			  USING SQLPIS;
	if sqlpis.sqlcode <> 0 then
		messagebox("확인","tloadplanhis child error : " + sqlpis.sqlerrtext)		
		lb_commit = false
		exit;
   else
		lb_commit = true
...
this.st_shipqty=create st_shipqty
this.st_minapqty=create st_minapqty
...
this.Control[iCurrent+5]=this.cb_srview
this.Control[iCurrent+6]=this.cb_split
this.Control[iCurrent+7]=this.cb_print
this.Control[iCurrent+8]=this.tab_1
this.Control[iCurrent+9]=this.dw_truck
this.Control[iCurrent+10]=this.cb_trucknew
this.Control[iCurrent+11]=this.dw_plancancel
this.Control[iCurrent+12]=this.st_2
this.Control[iCurrent+13]=this.st_3
this.Control[iCurrent+14]=this.st_4
this.Control[iCurrent+15]=this.st_shipqty
this.Control[iCurrent+16]=this.st_minapqty
this.Control[iCurrent+17]=this.st_invqty
this.Control[iCurrent+18]=this.dw_id
this.Control[iCurrent+19]=this.dw_truckorder
this.Control[iCurrent+20]=this.dw_print
this.Control[iCurrent+21]=this.sle_checksrno
this.Control[iCurrent+22]=this.em_shipeditno
this.Control[iCurrent+23]=this.sle_custcode
this.Control[iCurrent+24]=this.st_5
this.Control[iCurrent+25]=this.uo_shipoemgubun
this.Control[iCurrent+26]=this.gb_1
this.Control[iCurrent+27]=this.gb_2
this.Control[iCurrent+28]=this.gb_truck
this.Control[iCurrent+29]=this.gb_4
this.Control[iCurrent+30]=this.gb_5
this.Control[iCurrent+31]=this.gb_6
this.Control[iCurrent+32]=this.gb_8
this.Control[iCurrent+33]=this.gb_7
this.Control[iCurrent+34]=this.gb_9
this.Control[iCurrent+35]=this.gb_10
end on

on w_piss090u_back.destroy
call super::destroy
...
destroy(this.st_3)
destroy(this.st_4)
destroy(this.st_shipqty)
destroy(this.st_minapqty)
...
event ue_retrieve;call super::ue_retrieve;integer 	li_insert = 1, li_rtn, li_i
...
type uo_status from w_ipis_sheet01`uo_status within w_piss090u_back
integer y = 2576
end type
...
tab_1.tabpage_1.dw_normal.reset()
tab_1.tabpage_2.dw_minap.reset()
tab_1.tabpage_3.dw_sennap.reset()
tab_1.tabpage_4.dw_move.reset()
dw_truckorder.reset()
end event
...
event doubleclicked;wf_truckloadview(xpos, ypos, row, dwo)
end event
...
event clicked;il_delete_truckorder = dw_truck.object.truck[row]
...
event dragdrop;long        ll_truckorder
DragObject	ldo_object
DataWindow	ldw_control
