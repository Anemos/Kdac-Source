$PBExportHeader$w_bom116i.srw
$PBExportComments$M-BOM 조회(견적)
forward
global type w_bom116i from w_origin_sheet02
end type
type st_3 from statictext within w_bom116i
end type
type sle_itno from singlelineedit within w_bom116i
end type
type st_4 from statictext within w_bom116i
end type
type st_5 from statictext within w_bom116i
end type
type st_6 from statictext within w_bom116i
end type
type sle_ininv from singlelineedit within w_bom116i
end type
type sle_outinv from singlelineedit within w_bom116i
end type
type ddlb_choice from dropdownlistbox within w_bom116i
end type
type pb_itemfind from picturebutton within w_bom116i
end type
type st_8 from statictext within w_bom116i
end type
type uo_today from uo_today_bom within w_bom116i
end type
type pb_excel from picturebutton within w_bom116i
end type
type pb_print from picturebutton within w_bom116i
end type
type dw_print from datawindow within w_bom116i
end type
type uo_plant from uo_plandiv_bom within w_bom116i
end type
type tab_history from tab within w_bom116i
end type
...
type st_7 from statictext within w_bom116i
end type
type sle_osinv from singlelineedit within w_bom116i
end type
type st_1 from statictext within w_bom116i
end type
type rb_pre from radiobutton within w_bom116i
end type
type rb_post from radiobutton within w_bom116i
end type
type dw_condition_info1 from datawindow within w_bom116i
end type
type gb_2 from groupbox within w_bom116i
end type
type gb_3 from groupbox within w_bom116i
end type
type str_bomdata_info from structure within w_bom116i
end type
end forward
...
if	tab_history.selectedtab	=	1	then
	...
	if mid(ls_date,1,6) >= mid(g_s_date,1,6) then
		tab_history.tabpage_indent.dw_indent.dataobject	=	'd_bom116i_indent_current'
		tab_history.tabpage_indent.dw_indent.settransobject(sqlca)
		ln_count = 	tab_history.tabpage_indent.dw_indent.retrieve()
	else
		tab_history.tabpage_indent.dw_indent.dataobject	=	'd_bom116i_indent_history'
		tab_history.tabpage_indent.dw_indent.settransobject(sqlca)
...
type uo_status from w_origin_sheet02`uo_status within w_bom116i
end type
...
integer height = 2036
integer taborder = 80
string dataobject = "d_bom116i_indent_current"
...
for	i	=	1	to	rowcount
	ls_plant	=	this.object.toldplant[i]
	ls_div	=	this.object.tolddiv[i]	
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_bom116i_condition_info1"
