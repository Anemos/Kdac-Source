$PBExportHeader$w_mpm116u.srw
$PBExportComments$Work Calendar
forward
global type w_mpm116u from w_ipis_sheet01
end type
type dw_1 from u_vi_std_datawindow within w_mpm116u
end type
type dw_workgubun from datawindow within w_mpm116u
end type
type dw_print from datawindow within w_mpm116u
end type
type uo_month from u_mpms_date_scroll_month within w_mpm116u
end type
type gb_1 from groupbox within w_mpm116u
end type
end forward
...
//1
idw_cal_workgubun.SetTransObject(sqlmpms)
If idw_cal_workgubun.Retrieve(uo_month.is_uo_month) > 0 Then
	idw_primary.SetReDraw(False)
		...
		For li_i = 1 To 31 Step 1
			...
			ll_DayNo	= DaysAfter (Date( left(uo_month.is_uo_month, 4) + '.01.01'), Date(ls_date)) + 1
			  INSERT INTO TMPMCALENDAR  
						( ApplyMonth, ApplyDate, DayNo, WorkGubun, Remark, LastEmp, LastDate )  
...
type uo_status from w_ipis_sheet01`uo_status within w_mpm116u
end type
...
string dataobject = "d_mpm116u_01"
end type
...
string title = "none"
string dataobject = "d_mpm116u_02"
...
boolean titlebar = true
string title = "인쇄"
string dataobject = "d_mpm116u_01_print"
boolean hscrollbar = true
boolean vscrollbar = true
