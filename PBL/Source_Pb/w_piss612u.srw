...
type uo_label from u_piss_select_labelgubun within w_piss612u
end type
type st_6 from statictext within w_piss612u
end type
type sle_invoiceno from singlelineedit within w_piss612u
end type
type st_7 from statictext within w_piss612u
end type
type pb_print from picturebutton within w_piss612u
end type
end forward
...
string  ls_areacode,	ls_divisioncode,	ls_customercode, ls_message
...
select isnull(count(*),0) into :li_count from tshiplabelcontainer
where areacode				= :ls_areacode				and
		divisioncode		= :ls_divisioncode		and
		customercode		= :ls_customercode		and
...
public function integer wf_replacesave ();string ls_areacode, ls_divisioncode, ls_customercode, ls_customeritemcode,&
		 ls_invoiceno, ls_tracedate, ls_message
...
where areacode 			= :ls_areacode and
		...
		labelgubun			=  :is_labelgubun using sqlpis;

if sqlpis.sqlcode <>	0 then
	ls_message = "LabelMaster 수정 실패"
	goto RollBack_
end if	
...
public function integer wf_label_form (integer ag_printcount);string	ls_areacode,		ls_divisioncode,	ls_customercode, ls_customeritemcode,&
			...
			ls_customeritemcodebar, ls_labelgubun
...
For li_selectedcount = 1 To ll_rowcount
	...
	li_codecount = dw_5.retrieve(ls_areacode, ls_divisioncode, ls_customercode, &
				ls_customeritemcode,ls_invoiceno,li_serialnofrom,ls_labelgubun)
...
string	ls_serialtext, ls_message

select isnull(sum(labelcount),0) into :li_serialnofrom from tshiplabelmaster
...
if dw_3.dataobject = 'd_piss612u_03' then
	...
	insert	into tshiplabelmaster
		...
		values(:is_firstareacode,		:is_firstdivisioncode,		:is_firstcustomercode, :is_firstcustomeritemcode,
				 ...
				 :li_labelcount,			:li_shipqty, :ls_tracedate)  using sqlpis ;
	if sqlpis.sqlcode <> 0 then
...
event ue_retrieve;call super::ue_retrieve;string ls_invoiceno, ls_areacode, ls_divisioncode, ls_labelgubun, ls_customercode
...
ls_customercode = uo_customer.is_uo_customercode
ls_labelgubun = uo_label.is_uo_labelcode
ls_invoiceno = sle_invoiceno.text
...
l_n_row = dw_2.retrieve(ls_areacode, ls_divisioncode, ls_customercode, ls_labelgubun, ls_invoiceno)
...
event ue_select;If ib_open Then
	...
	f_pisc_retrieve_dddw_division(uo_division.dw_1, &
											...
											uo_division.is_uo_divisionname, &
											uo_division.is_uo_divisionnameeng)
	f_piss_retrieve_dddw_labelcustomer(uo_customer.dw_1,uo_area.is_uo_areacode,uo_division.is_uo_divisioncode,false,uo_customer.is_uo_customercode,uo_customer.is_uo_customername)
