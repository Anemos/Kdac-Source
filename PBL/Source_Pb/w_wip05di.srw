...
type rb_free from radiobutton within w_wip05di
end type
type rb_cost from radiobutton within w_wip05di
end type
end forward
...
if i_n_newindex = 1 then
	...
	if tab_1.tabpage_1.dw_wip05di_01.retrieve(g_s_company,l_s_vndr,i_s_year1,i_s_month1,ls_iocd) < 1 then
...
elseif i_n_newindex = 2 then
	...
	if tab_1.tabpage_2.dw_wip05di_02.retrieve(g_s_company,i_s_plant,i_s_dvsn,l_s_vndr2,i_s_year2,i_s_month2,ls_iocd) < 1 then
		uo_status.st_message.text = f_message("I020")
	else
		uo_status.st_message.text = f_message("I010")
	end if
