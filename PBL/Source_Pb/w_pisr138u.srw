$PBExportHeader$w_pisr138u.srw
$PBExportComments$사급품반출확인취소
forward
global type w_pisr138u from w_ipis_sheet01
end type
type uo_area from u_pisc_select_area within w_pisr138u
end type
type uo_division from u_pisc_select_division within w_pisr138u
end type
type dw_2 from u_vi_std_datawindow within w_pisr138u
end type
type dw_3 from u_vi_std_datawindow within w_pisr138u
end type
type st_hsplitbar from u_pism_splitbar within w_pisr138u
end type
type pb_1 from picturebutton within w_pisr138u
end type
type dw_scan from datawindow within w_pisr138u
end type
type uo_from from u_pisc_date_applydate within w_pisr138u
end type
type gb_1 from groupbox within w_pisr138u
end type
type gb_2 from groupbox within w_pisr138u
end type
type gb_3 from groupbox within w_pisr138u
end type
type gb_4 from groupbox within w_pisr138u
end type
end forward
...
ll_Row = dw_2.Retrieve(istr_partkb.areacode, istr_partkb.divcode, ls_carNo, uo_from.is_uo_date)
...
li_Net = MessageBox("반출확인", String(li_Cnt) + " 건의 반출증을 반출확인취소 하려고 합니다.~r~n" &
		                         + "~r~n반출 반출확인취소 하시겠습니까?", Question!, YesNo!, 2)
IF li_Net <> 1 THEN
	MessageBox("반출취소", "반출확인취소 되었습니다.", Information! )
	Return 
END IF
...
SetPointer(Arrow!)
MessageBox("취소완료", String(li_Cnt) + " 건의 반출증(송장) 반출취소처리가 완료되었습니다.", Information! )
...
sqlpis.AutoCommit = True
SetPointer(Arrow!)
MessageBox("취소실패", "반출증정보 Update에서 오류가 발생하였습니다.", StopSign! )

return 

end event

type uo_status from w_ipis_sheet01`uo_status within w_pisr138u
end type
...
integer taborder = 11
boolean bringtotop = true
string dataobject = "d_pisr138u_01"
