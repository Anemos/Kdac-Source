$PBExportHeader$w_piss903u.srw
$PBExportComments$상차계획(여주)
forward
global type w_piss903u from w_ipis_sheet01
end type
type cb_srview from commandbutton within w_piss903u
end type
type cb_split from commandbutton within w_piss903u
end type
type cb_print from commandbutton within w_piss903u
end type
type tab_1 from tab within w_piss903u
end type
...
type cb_trucknew from commandbutton within w_piss903u
end type
type dw_plancancel from datawindow within w_piss903u
end type
type dw_truckorder from datawindow within w_piss903u
end type
type sle_checksrno from singlelineedit within w_piss903u
end type
type em_shipeditno from editmask within w_piss903u
end type
type uo_shipoemgubun from u_pisc_select_code within w_piss903u
end type
type uo_area from u_pisc_select_area within w_piss903u
end type
type uo_division from u_pisc_select_division within w_piss903u
end type
type dw_truck from datawindow within w_piss903u
end type
type uo_date from u_piss_date_yeoju within w_piss903u
end type
type st_2 from statictext within w_piss903u
end type
type uo_date2 from u_piss_date_yeoju within w_piss903u
end type
type st_4 from statictext within w_piss903u
end type
type uo_scustgubun from u_pisc_select_code within w_piss903u
end type
type uo_custcode from u_piss_select_custcode within w_piss903u
end type
type st_5 from statictext within w_piss903u
end type
type gb_1 from groupbox within w_piss903u
end type
type gb_2 from groupbox within w_piss903u
end type
type gb_4 from groupbox within w_piss903u
end type
type gb_6 from groupbox within w_piss903u
end type
type gb_9 from groupbox within w_piss903u
end type
type gb_truck from groupbox within w_piss903u
end type
type gb_3 from groupbox within w_piss903u
end type
type gb_5 from groupbox within w_piss903u
end type
end forward
...
global w_piss903u w_piss903u

type variables
string is_areacode,is_divisioncode,is_date, is_today, is_filter[], is_delete_empno
...
string is_shipcount, is_custgubun,is_custcode = '%',  is_sort
...
public function boolean wf_update_tloadplantest_child (string fs_shipplandate, long fl_truckorder, string fs_srno, string fs_areacode, string fs_divisioncode, long fl_truckplan)
public subroutine wf_filter ()
public subroutine wf_truck_dw_clicked (integer xpos, integer ypos, long row, dwobject dwo)
public subroutine wf_truck_return_buffer (integer fi_truckorder, string fs_empno)
...
//		end if
		...
		OpenWithParm(w_piss9033, ls_parm)
...
OpenWithParm(w_piss9032, ls_parm)
end event
...
public subroutine wf_dw_clicked (integer xpos, integer ypos, long row, dwobject dwo);long	ll_clicked
...
public function boolean wf_update_tloadplantest_child (string fs_shipplandate, long fl_truckorder, string fs_srno, string fs_areacode, string fs_divisioncode, long fl_truckplan);long ll_parent_qty,ll_child_qty,ll_count,ll_shipeditno,ll_cnt
...
long ll_normal, ll_minap, ll_sennap, ll_move, ll_found, ll_sourcefind
string	ls_shipendgubun, ls_shipcount, ls_srno, ls_srgubun, ls_shipgubun, ls_find, ls_empno
integer	li_loadqty, li_dansuqty, li_rackqty, li_remainqty, li_shipendflag, li_truckorder, li_remainqty1
...
FOR ll_i = ll_normal_count TO 1 STEP -1
	...
	ls_empno		 	= trim(tab_1.tabpage_1.dw_normal.GetitemString(ll_i, "lastemp"))
	//Truckorder 가 0 이면 아직 상차계획을 한 제품이 아니므로
	if li_truckorder = fi_truckorder and ls_empno = fs_empno then
...
FOR ll_i = ll_minap_count TO 1 STEP -1
	...
	ls_empno		 	= trim(tab_1.tabpage_2.dw_minap.GetitemString(ll_i, "lastemp"))
	//Truckorder 가 0 이면 아직 상차계획을 한 제품이 아니므로
	if li_truckorder = fi_truckorder and ls_empno = fs_empno then
...
FOR ll_i = ll_sennap_count TO 1 STEP -1
	...
	ls_empno		 	= trim(tab_1.tabpage_3.dw_sennap.GetitemString(ll_i, "lastemp"))
	//Truckorder 가 0 이면 아직 상차계획을 한 제품이 아니므로
	if li_truckorder = fi_truckorder and ls_empno = fs_empno then
...
FOR ll_i = ll_move_count TO 1 STEP -1
	...
	ls_empno		 	= trim(tab_1.tabpage_4.dw_move.GetitemString(ll_i, "lastemp"))
	//Truckorder 가 0 이면 아직 상차계획을 한 제품이 아니므로
	if li_truckorder = fi_truckorder and ls_empno = fs_empno then
...
public function boolean wf_update_loadplan ();uo_status.st_message.text = 'tloadplantest delete 중(wf_update_tloadplantest)'	
long   	ll_truckorder, ll_loadqty, ll_dansuqty,ll_truckplan
Long		i, ll_rowcount,ll_shipeditno,ll_count
string	ls_srno, ls_custcode, ls_srgubun, ls_itemcode,ls_divisioncode,ls_shipgubun,ls_kitgubun,ls_custname
string   ls_modicheck
...
next
uo_status.st_message.text = 'tloadplantest delete 완료(wf_update_tloadplantest)'	
idw_normal.SetFilter('TruckOrder > 0')
idw_normal.Filter()
uo_status.st_message.text = 'tloadplantest 정상 저장 중(wf_update_tloadplantest)'	
ll_rowcount	= idw_normal.RowCount()
If ll_rowcount > 0 Then
	For i = 1 To ll_rowcount
		if trim(idw_normal.GetItemString(i, 'lastemp')) = g_s_empno then
			...
			INSERT INTO tloadplantest (shipplandate,areacode,divisioncode,srno,truckorder,itemcode,custcode,shipeditno,
			...
			VALUES ( :is_date,:is_areacode,:ls_divisioncode,:ls_srno,:ll_truckorder,:ls_itemcode,:ls_custcode,:ll_shipeditno,
					  :is_date,Null,:ll_truckplan,0,'N',:g_s_empno,GetDate())
					  using sqlpis;
...
//				messagebox("",'tlaodplan 정상 error : ' +sqlpis.sqlerrtext)
			...
			INSERT INTO tloadplantestHIS  
				...
				VALUES ( :is_date,:is_areacode,:ls_divisioncode,:ls_srno,:ll_truckorder,null,null,:ls_shipgubun,   
							:ls_itemcode,:ls_custcode,:ll_shipeditno,:is_date,:ll_truckplan,0,'N','N','N',null,
							:g_s_empno,getdate())
				using sqlpis;
			...
			If sqlpis.sqlcode <> 0 Then
				...
				exit
			End if			
			if ls_kitgubun = 'Y' then //KIT제품은 자품번도 tloadplantest에 저장
				if not wf_update_tloadplantest_child(is_date,ll_truckorder,ls_srno,is_areacode,ls_divisioncode,ll_truckplan) then
//				   SQLPIS.AUTOCOMMIT = TRUE					
...
If ll_rowcount > 0 Then
	For i = 1 To ll_rowcount
		if trim(idw_minap.GetItemString(i, 'lastemp')) = g_s_empno then
			...
			INSERT INTO tloadplantest (areacode,divisioncode,srno, truckorder,itemcode, custcode, shipplandate, shipeditno,
						applyfrom,Truckno,truckloadqty, truckdansuqty,truckmodifyflag, lastemp, lastdate)
...
If ll_rowcount > 0 Then
	For i = 1 To ll_rowcount
		if trim(idw_sennap.GetItemString(i, 'lastemp')) = g_s_empno then
...
If ll_rowcount > 0 Then
	For i = 1 To ll_rowcount
		if trim(idw_move.GetItemString(i, 'lastemp')) = g_s_empno then
...
long		ll_selected_rows[], 	ll_selected_count, ll_count,	ll_rowcount, ll_i, ll_find, ll_targetrow
long  	li_remainqty, li_rackqty, li_loadqty, li_shipplanqty, li_dansuqty, li_load, li_truckorder, li_remainqty1, ll_searchtruck
string	ls_srno, ls_find, ls_shipgubun,ls_empno
...
ls_find	= " empno = '" + g_s_empno + "' and  Truck = " + string(fi_truckorder)
ll_find  = dw_truck.Find(ls_find , 1, ll_rowcount)
if ll_find <= 0 then
	dw_truck.SetRedraw(False)
	dw_truck.InsertRow(0)
	dw_truck.Setitem(ll_rowcount + 1, 1, fi_truckorder)
	dw_truck.Setitem(ll_rowcount + 1, 3, g_s_empno)
	dw_truck.Sort()
	ls_find	= " empno = '" + g_s_empno + "' and  Truck = " + string(fi_truckorder)
	ll_searchtruck = dw_truck.Find(ls_find, 1, ll_rowcount + 1 )
...
FOR ll_i = ll_selected_count TO 1 STEP -1
	...
	if li_truckorder = 0 then		
		...
		if ls_itemclass = '10' and (ls_itembuysource = '05' or ls_itembuysource = '06' ) then
			...
			adw_source.SetItem(ll_selected_rows[ll_i], "lastemp", g_s_empno)
...
this.Control[iCurrent+1]=this.cb_srview
this.Control[iCurrent+2]=this.cb_split
this.Control[iCurrent+3]=this.cb_print
this.Control[iCurrent+4]=this.tab_1
this.Control[iCurrent+5]=this.cb_trucknew
this.Control[iCurrent+6]=this.dw_plancancel
this.Control[iCurrent+7]=this.dw_truckorder
this.Control[iCurrent+8]=this.sle_checksrno
this.Control[iCurrent+9]=this.em_shipeditno
...
this.Control[iCurrent+13]=this.dw_truck
this.Control[iCurrent+14]=this.uo_date
this.Control[iCurrent+15]=this.st_2
this.Control[iCurrent+16]=this.uo_date2
this.Control[iCurrent+17]=this.st_4
this.Control[iCurrent+18]=this.uo_scustgubun
this.Control[iCurrent+19]=this.uo_custcode
...
event ue_retrieve;integer 	li_insert = 1, li_rtn, li_i , ln_find = 1
...
If idw_normal.Retrieve(is_areacode,is_divisioncode,is_today) > 0 Then
	idw_normal.Post Event Clicked(1,1,1,ldo_object)
End If
uo_status.st_message.text = '미납 조회 중'
If idw_minap.Retrieve(is_areacode,is_divisioncode,is_today) > 0 Then
	idw_minap.Post Event Clicked(1,1,1,ldo_object)
End If
uo_status.st_message.text = '선납 조회 중'
If idw_sennap.Retrieve(is_areacode,is_divisioncode,is_today) > 0 Then
	idw_sennap.Post Event Clicked(1,1,1,ldo_object)
End If
uo_status.st_message.text = '이체 조회 중'
If idw_move.Retrieve(is_areacode,is_divisioncode,is_today) > 0 Then
...
If ll_rowcount > 0 Then
	...
	If dw_truckorder.Find('TruckFlag = 0', 1, ll_rowcount) > 0 Then
		...
		For li_i = 1 To ll_rowcount
			...
			If dw_truckorder.GetitemNumber(li_i, 'truckflag') = 1 Then
				dw_truck.InsertRow(li_insert)
				dw_truck.SetItem(li_insert, 1, dw_truckorder.GetitemNumber(li_i, 'truckorder'))
				dw_truck.SetItem(li_insert, 3, dw_truckorder.GetitemString(li_i, 'lastemp'))
		...
		If li_insert = 1 Then //만일 모두가 다 출하된 것이라면 
			dw_truck.InsertRow(li_insert)
			dw_truck.SetItem(li_insert, 1, ii_maxtruck + 1 )
			dw_truck.SetItem(li_insert, 3, g_s_empno)
	...
	Else// 상차된 차량은 있으나 출하된 차량이 없다면 
		For li_i = 1 To ll_rowcount
			...
			dw_truck.SetItem(li_i, 3, dw_truckorder.GetitemString(li_i, 'lastemp'))
...
// 기존에 상차계획된 차량이 한개도 없다면 무조건 1번 차량 부터
	...
	dw_truck.SetItem(1, 3, g_s_empno)
End If
...
event ue_print;call super::ue_print;string ls_date, ls_truckorder, ls_parm, ls_size, ls_empno
...
//If row > 0 Then
	...
	ls_empno			= trim(dw_truck.GetitemString(il_old_row, 'empno') )
	ls_size	= Left(String(WorkSpaceX()) + Space(10), 10)	+ &
				  Left(String(WorkSpaceY()) + Space(10), 10)	+ &
				  Left(String(WorkSpaceWidth()) + Space(10), 10) + &
				  Left(String(WorkSpaceHeight()) + Space(10), 10)
	...
	ls_parm 			= is_areacode + is_divisioncode + Left(ls_date + Space(10), 10 ) + &
						  Left(string(ls_truckorder) + Space(10), 10 ) + Left(ls_empno + Space(06), 06 ) + ls_size
						  
	OpenWithParm(w_piss9031, ls_parm)
	dw_truck.setredraw(True)
end if
...
type uo_status from w_ipis_sheet01`uo_status within w_piss903u
integer x = 18
integer y = 2404
end type
...
string title = "none"
string dataobject = "d_piss903u_01"
...
string dragicon = "WinLogo!"
string title = "none"
string dataobject = "d_piss903u_02"
boolean hscrollbar = true
boolean vscrollbar = true
boolean hsplitscroll = true
boolean livescroll = true
...
string dragicon = "WinLogo!"
string title = "none"
string dataobject = "d_piss903u_03"
boolean hscrollbar = true
boolean vscrollbar = true
boolean hsplitscroll = true
boolean livescroll = true
...
string dragicon = "WinLogo!"
string title = "none"
string dataobject = "d_piss903u_04"
boolean hscrollbar = true
boolean vscrollbar = true
boolean hsplitscroll = true
boolean livescroll = true
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_piss903u_06"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
//	wf_truck_return_buffer(ll_truckorder,is_delete_empno)
end if
end event
...
string dataobject = "d_piss903u_05"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
event clicked;if row > 0 then
	...
	is_delete_empno		= dw_truck.object.empno[row]
...
on uo_date.destroy
call u_piss_date_yeoju::destroy
end on
...
event constructor;call super::constructor;is_today = is_uo_date
end event

event ue_losefocus;call super::ue_losefocus;is_today = is_uo_date
end event

event ue_select;call super::ue_select;is_today = is_uo_date
end event
