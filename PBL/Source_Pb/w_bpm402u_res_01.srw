$PBExportHeader$w_bpm402u_res_01.srw
$PBExportComments$하위품번 복사 Response Window
forward
global type w_bpm402u_res_01 from window
end type
type p_1 from picture within w_bpm402u_res_01
end type
type cb_2 from commandbutton within w_bpm402u_res_01
end type
type cb_1 from commandbutton within w_bpm402u_res_01
end type
type sle_2 from singlelineedit within w_bpm402u_res_01
end type
type sle_1 from singlelineedit within w_bpm402u_res_01
end type
type st_1 from statictext within w_bpm402u_res_01
end type
type dw_1 from datawindow within w_bpm402u_res_01
end type
type gb_1 from groupbox within w_bpm402u_res_01
end type
type gb_2 from groupbox within w_bpm402u_res_01
end type
end forward
...
global w_bpm402u_res_01 w_bpm402u_res_01
		...
		if l_n_cnt > 0 then
			...
			for i = 1 to l_n_rows
				...
				if l_s_pcitn = l_s_itno then
					...
					if f_chk_bom_bpm(l_s_applyyear,l_s_revno,l_s_plant,l_s_div,l_s_pcitn,l_s_itno,l_s_applydate) = 1 then	
						messagebox("확인","BOM 구조를 확인하세요.")
						return
					end if
				end if
...
f_bpm_job_start(l_s_applyyear,l_s_revno,'w_bpm402u',g_s_empno,'C','사업계획BOM정보 하위품번복사')
for i = 1 to l_n_rows
	l_d_plant = l_s_plant
   l_d_div   = l_s_div
	l_d_ppitn = l_s_pcitn
	...
	INSERT INTO "PBBPM"."BPM504"  
         ( "PCMCD","XYEAR","REVNO","PLANT","PDVSN","PPITN","PROUT","PCITN","PCHDT","PQTYM","PQTYE", "PWKCT","PEDTM",   
           "PEDTE","POPCD","PEXPLANT","PEXDV","PCHCD","POSCD","PEBST",
  			...
  			VALUES (:g_s_company,:l_s_applyyear,:l_s_revno,:l_d_plant,:l_d_div,:l_d_ppitn,:l_d_prout,:l_d_pcitn,:l_d_pchdt,:l_d_pqtym,:l_d_pqtye
...
integer height = 628
integer taborder = 10
string dataobject = "d_bpm504_explo_02"
