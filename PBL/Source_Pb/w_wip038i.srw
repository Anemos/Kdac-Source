$PBExportHeader$w_wip038i.srw
$PBExportComments$재공수불현황(공제분)
forward
global type w_wip038i from w_origin_sheet04
end type
type dw_1 from datawindow within w_wip038i
end type
type dw_2 from datawindow within w_wip038i
end type
type dw_3 from datawindow within w_wip038i
end type
type pb_1 from picturebutton within w_wip038i
end type
type gb_1 from groupbox within w_wip038i
end type
end forward
...
//		ls_orct = 'D%'
//	else
//		ls_orct   = dw_3.getitemstring(1,"wip001_waorct")
//	end if
//end if
...
if i_s_iocd = '1' then
	dw_1.dataobject = 'd_wip038i_linelist'
	dw_1.settransobject(sqlca)
	l_n_rowcnt = dw_1.retrieve(g_s_company, ls_plant, ls_dvsn, i_s_iocd, i_s_fromdt, i_s_todt, i_s_adddt)
	dw_2.dataobject = 'd_wip038i_linerpt'
	dw_2.settransobject(sqlca)
elseif i_s_iocd = '2' then
	dw_1.dataobject = 'd_wip038i_vndlist'
	dw_1.settransobject(sqlca)
	l_n_rowcnt = dw_1.retrieve(g_s_company, ls_plant, ls_dvsn, i_s_iocd, i_s_fromdt, i_s_todt, i_s_adddt)
   dw_2.dataobject = 'd_wip038i_vndrpt'
	dw_2.settransobject(sqlca) 
else
	dw_1.dataobject = 'd_wip038i_stklist'
	dw_1.settransobject(sqlca)
	l_n_rowcnt = dw_1.retrieve(g_s_company, ls_plant, ls_dvsn, i_s_fromdt, i_s_todt, i_s_adddt)
   dw_2.dataobject = 'd_wip038i_stkrpt'
	dw_2.settransobject(sqlca)
end if
...
type uo_status from w_origin_sheet04`uo_status within w_wip038i
end type
...
string dataobject = "d_wip038i_linelist"
...
event retrieveend;long ll_cnt
dec{5} lc_avrg

if dw_1.dataobject = 'd_wip038i_stklist' then
	for ll_cnt = 1 to rowcount
		This.setitem(ll_cnt,"usat9",this.getitemdecimal(ll_cnt,"bgat") + this.getitemdecimal(ll_cnt,"inat") &
	 		- ( this.getitemdecimal(ll_cnt,"usat1") + this.getitemdecimal(ll_cnt,"usat2") + &
			 	 this.getitemdecimal(ll_cnt,"usat3") + this.getitemdecimal(ll_cnt,"usat4") + &
				 this.getitemdecimal(ll_cnt,"usat5") + this.getitemdecimal(ll_cnt,"usat6") + &
				 this.getitemdecimal(ll_cnt,"usat8") + this.getitemdecimal(ll_cnt,"ohat") ) )
	next
else
	for ll_cnt = 1 to rowcount
		This.setitem(ll_cnt,"usat9",this.getitemdecimal(ll_cnt,"bgat") + this.getitemdecimal(ll_cnt,"inat") &
	 		- ( this.getitemdecimal(ll_cnt,"usat1") + this.getitemdecimal(ll_cnt,"usat2") + &
			 	 this.getitemdecimal(ll_cnt,"usat3") + this.getitemdecimal(ll_cnt,"usat4") + &
				 this.getitemdecimal(ll_cnt,"usat5") + this.getitemdecimal(ll_cnt,"usat6") + &
				 this.getitemdecimal(ll_cnt,"usat7") + this.getitemdecimal(ll_cnt,"usat8") + &
				 this.getitemdecimal(ll_cnt,"usata") + this.getitemdecimal(ll_cnt,"ohat") ) )
	next
end if
end event
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_wip038i_linerpt"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_wip038i_01"
