$PBExportHeader$u_seq_select_productgroup.sru
$PBExportComments$제품군 선택 - is_uo_productgroup, is_uo_productgroupname
forward
global type u_seq_select_productgroup from userobject
end type
type dw_1 from datawindow within u_seq_select_productgroup
end type
type st_1 from statictext within u_seq_select_productgroup
end type
end forward
...
global u_seq_select_productgroup u_seq_select_productgroup
...
on u_seq_select_productgroup.create
...
on u_seq_select_productgroup.destroy
destroy(this.dw_1)
destroy(this.st_1)
end on
...
integer height = 68
integer taborder = 10
string dataobject = "d_seq_dddw_productgroup"
...
If is_uo_productgroup <> ls_productgroup Then
	...
	Else
		...
		If GetChild('DDDWCode', ldwc_1) = 1 Then
			...
			If ll_find > 0 Then
				is_uo_productgroupname	 = Trim(ldwc_1.GetitemString(ll_find, 'productName'))
			End If			
		End If
	End If
