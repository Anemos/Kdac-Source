...
If dw_1.Retrieve(	uo_area.is_uo_areacode,				uo_division.is_uo_divisioncode, &
						"%") > 0 Then
	uo_status.st_message.text =  "단말기마스터 정보" //+ f_message("변경된 데이타가 없습니다.")
Else
	uo_status.st_message.text =  "단말기마스터 정보가 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
	MessageBox("단말기마스터", "단말기마스터 정보가 존재하지 않습니다")
End If
...
If MessageBox("단말기마스터", "변경된 단말기마스터 정보를 저장하시겠습니까?", &
...
If SQLPIS.sqlcode = 0 Then
	...
	MessageBox("단말기마스터", "변경된 단말기마스터를 저장하였습니다.", Information!)
...
Else
	...
	SQLPIS.AutoCommit = True
	MessageBox("단말기마스터", "변경된 단말기마스터를 저장하는 중에 오류가 발생하였습니다.", StopSign!)
End If
...
If dw_1.GetRow() > 0 Then
	//
Else
	MessageBox("단말기마스터", "삭제하시려는 단말기마스터를 선택하십시오.", Exclamation!)
	Return
End If
...
If ll_count > 0 Then
	If MessageBox("단말기마스터", "선택하신 단말기는 단말기/라인 관리 마스터에 등록되어 있습니다." + &
									"~r~n~r~n선택하신 단말기마스터를 삭제하시겠습니까?", &
									Question!, YesNo!, 2) = 2 Then
		Return
	End If
Else
	If MessageBox("단말기마스터", "선택하신 단말기마스터 정보를 삭제하시겠습니까?", &
												Question!, YesNo!, 2) = 2 Then
		Return
	End If
End If
...
If SQLPIS.sqlcode = 0 Then
	Commit Using SQLPIS;
	SQLPIS.AutoCommit = True
	MessageBox("단말기마스터", "선택하신 단말기마스터를 삭제하였습니다.", Information!)
	iw_this.TriggerEvent("ue_retrieve")
Else
	RollBack Using SQLPIS;
	SQLPIS.AutoCommit = True
	MessageBox("단말기마스터", "선택하신 단말기마스터를 삭제하는 중에 오류가 발생하였습니다.", StopSign!)
End If
