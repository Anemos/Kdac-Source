$PBExportHeader$w_bom201i.srw
$PBExportComments$M-BOM 조회(월별)
forward
global type w_bom201i from w_origin_sheet02
end type
type pb_excel from picturebutton within w_bom201i
end type
type uo_1 from uo_plandiv_pdcd within w_bom201i
end type
type st_1 from statictext within w_bom201i
end type
type uo_date from uo_ccyymm_mps within w_bom201i
end type
type st_2 from statictext within w_bom201i
end type
type sle_1 from singlelineedit within w_bom201i
end type
type pb_1 from picturebutton within w_bom201i
end type
type dw_bom_print from datawindow within w_bom201i
end type
type tab_1 from tab within w_bom201i
end type
...
type st_4 from statictext within w_bom201i
end type
type rb_pre from radiobutton within w_bom201i
end type
type rb_post from radiobutton within w_bom201i
end type
type st_5 from statictext within w_bom201i
end type
type st_itnm from statictext within w_bom201i
end type
type gb_1 from groupbox within w_bom201i
end type
end forward
...
ls_applydate = f_relativedate(f_relative_month(ls_yyyymm + '01',1),-1)
...
this.Control[iCurrent+8]=this.dw_bom_print
this.Control[iCurrent+9]=this.tab_1
this.Control[iCurrent+10]=this.st_4
this.Control[iCurrent+11]=this.rb_pre
this.Control[iCurrent+12]=this.rb_post
this.Control[iCurrent+13]=this.st_5
this.Control[iCurrent+14]=this.st_itnm
this.Control[iCurrent+15]=this.gb_1
end on

on w_bom201i.destroy
call super::destroy
...
string  ls_plant,ls_div,ls_pdcd,ls_itno,ls_rtncd,ls_yyyymm,ls_srce,ls_applygubun,ls_message
ls_rtncd 	= uo_1.uf_return()
ls_yyyymm 	= uo_date.uf_yyyymm()
...
if tab_1.selectedtab = 1 then
	...
	if tab_1.tabpage_1.dw_bom.retrieve(ls_yyyymm,ls_plant,ls_div,ls_pdcd,ls_itno,ls_applygubun) < 1 then
...
elseif tab_1.selectedtab = 2 then
	...
	Choose Case	tab_1.tabpage_2.ddlb_srce.text
		...
		case else
			ls_srce	=	'%'
	End Choose		
	if tab_1.tabpage_2.dw_knockdown.retrieve(ls_yyyymm,ls_plant,ls_div,ls_pdcd,ls_srce,ls_itno) < 1 then
		uo_status.st_message.text = f_message("I020")
		pb_excel.enabled  = false
		pb_excel.visible  = false
	else
...
elseif tab_1.selectedtab = 3 then
	...
	if tab_1.tabpage_3.dw_partlist_fta.retrieve(ls_yyyymm,ls_plant,ls_div,ls_itno,ls_applygubun) < 1 then
		//생성로직 추가
...
if tab_1.selectedtab = 1 then
	...
	string	ls_plant,ls_div,ls_pdcd,ls_itno,ls_rtncd,ls_yyyymm,ls_applygubun
	...
	dw_bom_print.reset()
	if dw_bom_print.retrieve(ls_yyyymm,ls_plant,ls_div,ls_pdcd,ls_itno,string(g_s_date,'@@@@.@@.@@'),ls_applygubun) < 1 then
...
type uo_status from w_origin_sheet02`uo_status within w_bom201i
integer y = 2468
end type
...
event ue_modify;call super::ue_modify;tab_1.tabpage_1.dw_bom.reset()
end event
...
integer taborder = 50
boolean bringtotop = true
string dataobject = "d_bom201_print"
...
integer height = 1936
integer taborder = 80
string dataobject = "d_bom201_retrieve"
...
integer height = 1812
integer taborder = 90
string dataobject = "d_bom201_knockdown"
boolean hscrollbar = true
boolean vscrollbar = true
boolean hsplitscroll = true
boolean livescroll = true
...
integer height = 1940
integer taborder = 80
string dataobject = "d_bom201_partlist_fta_allproduct"
boolean hscrollbar = true
boolean vscrollbar = true
boolean hsplitscroll = true
boolean livescroll = true
