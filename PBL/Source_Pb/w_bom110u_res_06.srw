$PBExportHeader$w_bom110u_res_06.srw
$PBExportComments$무상 <-> 유상 전환 하위조회
forward
global type w_bom110u_res_06 from window
end type
type em_today from editmask within w_bom110u_res_06
end type
type cb_cancel from commandbutton within w_bom110u_res_06
end type
type cb_ok from commandbutton within w_bom110u_res_06
end type
type sle_itno from singlelineedit within w_bom110u_res_06
end type
type st_1 from statictext within w_bom110u_res_06
end type
type dw_1 from datawindow within w_bom110u_res_06
end type
type gb_1 from groupbox within w_bom110u_res_06
end type
type gb_2 from groupbox within w_bom110u_res_06
end type
end forward
...
global w_bom110u_res_06 w_bom110u_res_06
...
event clicked;string 		ld_prout, ld_ppitn, ld_pcitn, ld_pchdt, ld_pwkct, ld_pedtm,ld_oldpchdt,  &
...
//	end if
	...
	if ls_itclsb = "40" or ls_itclsb = "50" then
		if ld_pwkct <> "9999" then
			messagebox("확인", "하위품번 : " + ld_ppitn + " 조코드를 확인해 주시기 바랍니다.")
			return -1
		end if
	end if
...
for i = 1 to in_rows
	if	dw_1.getitemstatus(i,"pwkct",primary!)	=	DataModified!	then
		...
		where		pcmcd	   =	:g_s_company	and	plant	=	:is_plant	and	pdvsn	=	:is_div		and	prout	=	:ld_prout	and
					ppitn		=	:ld_ppitn		and	pcitn	=	:ld_pcitn	and	pchdt	=	:ld_oldpchdt
		using	sqlca ;
		...
		VALUES (	:g_s_company,:is_plant,:is_div,:ld_ppitn ,:ld_prout,:ld_pcitn,:ld_pchdt,:ld_pqtym,:ld_pqtye,
		  			:ld_pwkct,:ld_pedtm,:ld_pedte,'',:ld_pexplant,:ld_pexdv,'A',:ld_poscd,:ld_pebst,
		  			:g_s_macaddr,:g_s_ipaddr,:g_s_date,:g_s_empno,'')
		using sqlca;
		if sqlca.sqlcode <> 0 then
			messagebox("확인","하위품번 : " + ld_pcitn + " BOM 입력 오류")
			return -1
		end if
		//		승인 Process 추가 - 2007.12.17
		if	f_bom_approve(is_plant,is_div,ld_ppitn,ld_pcitn,'B','S',dw_1,ld_pedtm,i)	<>	0	then
...
integer height = 640
integer taborder = 20
string dataobject = "d_bom001_explo_06"
