$PBExportHeader$w_pisk120i.srw
$PBExportComments$간판준수율 - 월별
forward
global type w_pisk120i from w_ipis_sheet01
end type
type dw_1 from u_vi_std_datawindow within w_pisk120i
end type
type uo_area from u_pisc_select_area within w_pisk120i
end type
type uo_division from u_pisc_select_division within w_pisk120i
end type
type uo_workcenter from u_pisc_select_workcenter within w_pisk120i
end type
type uo_line from u_pisc_select_line within w_pisk120i
end type
type dw_detail from datawindow within w_pisk120i
end type
type st_h_bar from uo_xc_splitbar within w_pisk120i
end type
type uo_year from u_pisc_date_scroll_year within w_pisk120i
end type
type dw_data from datawindow within w_pisk120i
end type
type dw_print from datawindow within w_pisk120i
end type
type gb_1 from groupbox within w_pisk120i
end type
type gb_2 from groupbox within w_pisk120i
end type
type gb_3 from groupbox within w_pisk120i
end type
end forward
...
global w_pisk120i w_pisk120i

type variables
Boolean	ib_open
end variables

on w_pisk120i.create
int iCurrent
call super::create
...
//
//	uo_status.st_message.text =  "월별 간판 준수율 정보" //+ f_message("변경된 데이타가 없습니다.")
//Else
//	uo_status.st_message.text =  "월별 간판 준수율 정보가 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
//	MessageBox("월별 간판 준수율", "월별 간판 준수율 정보가 존재하지 않습니다")
//End If
...
Long		i, j, ll_rowcount, &
			...
			ll_release[12], ll_prd[12], ll_different[12], &
			ll_release_all_sub, ll_prd_all_sub, ll_different_all_sub, &
			ll_release_sub[12], ll_prd_sub[12], ll_different_sub[12], &
			ll_release_all_total, ll_prd_all_total, ll_different_all_total, &
			ll_release_total[12], ll_prd_total[12], ll_different_total[12]
...
lstr_prt.title			= '월별 간판 준수율'
lstr_prt.tag			= '월별 간판 준수율'
lstr_prt.dwsyntax		= ls_mod
OpenSheetWithParm(w_prt, lstr_prt, w_frame, 0, Layered!)

end event

type uo_status from w_ipis_sheet01`uo_status within w_pisk120i
end type
...
event rowfocuschanged;//string	ls_areacode, ls_divisioncode, ls_workcenter, ls_linecode
//
...
string title = "none"
string dataobject = "d_pisk120i_01"
boolean hscrollbar = true
boolean vscrollbar = true
