$PBExportHeader$w_pisp051i.srw
$PBExportComments$일일 생산계획(라인별)
forward
global type w_pisp051i from w_ipis_sheet01
end type
type dw_1 from u_vi_std_datawindow within w_pisp051i
end type
type uo_area from u_pisc_select_area within w_pisp051i
end type
type uo_division from u_pisc_select_division within w_pisp051i
end type
type uo_date from u_pisc_date_applydate within w_pisp051i
end type
type dw_print from datawindow within w_pisp051i
end type
type uo_workcenter from u_pisc_select_workcenter within w_pisp051i
end type
type uo_line from u_pisc_select_line within w_pisp051i
end type
type gb_1 from groupbox within w_pisp051i
end type
type gb_2 from groupbox within w_pisp051i
end type
type gb_3 from groupbox within w_pisp051i
end type
end forward
...
this.Control[iCurrent+6]=this.uo_workcenter
this.Control[iCurrent+7]=this.uo_line
this.Control[iCurrent+8]=this.gb_1
this.Control[iCurrent+9]=this.gb_2
this.Control[iCurrent+10]=this.gb_3
end on

on w_pisp051i.destroy
call super::destroy
destroy(this.dw_1)
...
If dw_1.Retrieve(ls_todate, &
	...
	uo_status.st_message.text =  "일일생산계획(라인별) 정보" //+ f_message("변경된 데이타가 없습니다.")
Else
	uo_status.st_message.text =  "일일생산계획(라인별)이 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
	MessageBox("일일생산계획(라인별)", "일일생산계획(라인별)이 존재하지 않습니다")
End If
end event
...
lstr_prt.title			= '일일생산계획(라인별)'
lstr_prt.tag			= '일일생산계획(라인별)'
...
type uo_status from w_ipis_sheet01`uo_status within w_pisp051i
end type
...
string dataobject = "d_pisp051i_01"
...
string title = "인쇄"
string dataobject = "d_pisp051i_01_print"
boolean hscrollbar = true
boolean vscrollbar = true
