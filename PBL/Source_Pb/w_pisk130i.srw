$PBExportHeader$w_pisk130i.srw
$PBExportComments$평준화준수율 - 일별
forward
global type w_pisk130i from w_ipis_sheet01
end type
type dw_1 from u_vi_std_datawindow within w_pisk130i
end type
type uo_area from u_pisc_select_area within w_pisk130i
end type
type uo_division from u_pisc_select_division within w_pisk130i
end type
type uo_workcenter from u_pisc_select_workcenter within w_pisk130i
end type
type uo_line from u_pisc_select_line within w_pisk130i
end type
type dw_detail from datawindow within w_pisk130i
end type
type st_h_bar from uo_xc_splitbar within w_pisk130i
end type
type uo_month from u_pisc_date_scroll_month within w_pisk130i
end type
type dw_data from datawindow within w_pisk130i
end type
type dw_print from datawindow within w_pisk130i
end type
type gb_1 from groupbox within w_pisk130i
end type
type gb_2 from groupbox within w_pisk130i
end type
type gb_3 from groupbox within w_pisk130i
end type
end forward
...
global w_pisk130i w_pisk130i

type variables
Boolean	ib_open
end variables

on w_pisk130i.create
int iCurrent
call super::create
...
//
//	uo_status.st_message.text =  "일별 평준화 준수율 정보" //+ f_message("변경된 데이타가 없습니다.")
//Else
//	uo_status.st_message.text =  "일별 평준화 준수율 정보가 존재하지 않습니다" //+ f_message("변경된 데이타가 없습니다.")
//	MessageBox("일별 평준화 준수율", "일별 평준화 준수율 정보가 존재하지 않습니다")
//End If
...
Long		i, j, ll_rowcount, &
			...
			ll_release[31], ll_prd[31], &
			ll_release_all_sub, ll_prd_all_sub, &
			ll_release_sub[31], ll_prd_sub[31],  &
			ll_release_all_total, ll_prd_all_total, &
			ll_release_total[31], ll_prd_total[31]
...
If ll_rowcount > 0 Then
	//
Else
	MessageBox("일별 간판 평준화율", "일별 간판 평준화율 정보가 존재하지 않습니다.", &
												Exclamation!)
	Return
End If
...
lstr_prt.title			= '일별 간판 평준화율'
lstr_prt.tag			= '일별 간판 평준화율'
lstr_prt.dwsyntax		= ls_mod
OpenSheetWithParm(w_prt, lstr_prt, w_frame, 0, Layered!)

end event

type uo_status from w_ipis_sheet01`uo_status within w_pisk130i
end type
...
string title = "none"
string dataobject = "d_pisk130i_01"
boolean hscrollbar = true
boolean vscrollbar = true
