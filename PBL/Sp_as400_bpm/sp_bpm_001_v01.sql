...
declare bpm501a_cursor Cursor for
 ...
 from pbbpm.bpm501 a left outer join pbbpm.bpm502 b
    on a.ayear = b.xyear and a.amdno = b.itno
   left outer join pbbpm.bpm503 c
    on a.aplant = c.xplant and a.adiv = c.div and
       a.amdno = c.itno and a.ayear = c.xyear
...
declare bpm501b_cursor cursor for
  select a.tplnt,a.tdvsn,a.tpitn,a.tcitn,a.tlevel,
    ...
    a.toption,a.tcalculate,a.tedtm,a.tedte
  from qtemp.tmp_bpm a left outer join pbbpm.bpm502 b
    on b.xyear = a_xyear and a.tcitn = b.itno
    left outer join pbbpm.bpm503 c
    on c.xyear = a_xyear and a.tplnt = c.xplant and
...
loop
  ...
  if p_cls = '35' or p_srce = '01' or
    ...
    ( comltd,xyear,brev,bgubun,bplant,bdiv,bmdcd,bmdno,
      ...
      bfmdt,btodt,balcd,byou,bgrad,baltno,bcalc,
      extd,inptid,inptdt,updtid,updtdt,ipaddr,macaddr )
    values(a_comltd,a_xyear,a_rev,a_divcode,a_plant,a_dvsn,p_mdcd,p_mdno,
      5,p_mdno,p_mdno,1,5,p_xunit,'',p_wkprqt,p_wkprqt,
      a_applydate,a_applydate,a_plant,'','','','Y',
      '','','',a_lastemp,a_lastdate,'','');
  end if;

  set p_rtn = pbbpm.sf_bpm_exp(a_comltd,a_xyear,a_plant,a_dvsn,
  ...
  loop
    fetch bpm501b_cursor into p_plant,p_dvsn,p_prno,p_chno,p_lev,
      ...
      p_xunit,p_cls,p_srce,p_grad,p_altno,p_calc,p_fmdt,p_todt;
    ...
    if p_altno = '*' then
      ...
      where ocmcd = a_comltd and oplant = p_plant and
      odvsn = p_dvsn and opitn = p_chno
      and xyear = a_xyear
      and (( oedte = ' ' and  oedtm <= a_applydate ) or
    ...
    values(a_comltd,a_xyear,a_rev,a_divcode,a_plant,a_dvsn,p_mdcd,p_mdno,
      ...
      p_fmdt,p_todt,p_alcd,p_you,p_grad,p_altno,p_calc,
      '','','',a_lastemp,a_lastdate,'','');
