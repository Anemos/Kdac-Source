...
declare bpmchk_cur02 cursor for
  select tplnt,tdvsn,tcitn,twkct,tqty1,texplant,
         texdv,topcd,tserl,tcalculate,toscd
  from qtemp.tmp_bpm
...
select srce, cls into p_srce, p_itcl from pbbpm.bpm503
  where xplant = a_plant and div = a_dvsn and
    itno = p_pitno and xyear = a_xyear;
if sqlcode <> 0 then
  return 'N';
end if;
...
if p_opcd = '2' and p_comcd = 'Y' then
  ...
  from pbbpm.bpm503
  where xyear = a_xyear and xplant = p_plant and
...
loop
 fetch bpmchk_cur02 into p_plant,p_dvsn,p_citno,p_pwkct,
   p_qty,p_explant,p_exdvsn,p_popcd,p_serl,p_parentcal,p_poscd;
 ...
 where xyear = a_xyear and xplant = p_beforeplant and
