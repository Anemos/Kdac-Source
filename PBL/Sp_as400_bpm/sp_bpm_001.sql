...
-- 배분후('A') - BOM전개('C' : 호환무시 공장별전개)
-- 배분전('B') - BOM전개('A' : 호환무시 전공장전개)
-- 사업계획 매출 버전 a_rev ('0A'~ )
...
declare bpm501b_cursor cursor for
  select a.tplnt,a.tdvsn,a.tpitn,a.tcitn,a.tlevel,
    ...
    a.torate,a.tcumorate,a.topcd,a.toption,a.topchk
...
loop
  ...
  if p_cls = '35' or p_srce = '01' or
    ...
    ( comltd,xyear,brev,bgubun,bplant,bdiv,bmdcd,bmdno,
      ...
      bygchk,borate,bcumorate,extd,updtid,updtdt,
      bopcd,boption,bopchk )
    values(a_comltd,a_xyear,a_rev,a_divcode,a_plant,a_dvsn,p_mdcd,p_mdno,
      ...
      p_ygchk,1,1,'',a_lastemp,a_lastdate,'','','');
  ...
  loop
    fetch bpm501b_cursor into p_plant,p_dvsn,p_prno,p_chno,p_lev,
      ...
      p_fmdt,p_todt,p_comcd,p_serl,p_ygchk,p_orate,p_cumorate,
      p_opcd,p_option,p_opchk;
    ...
    values(a_comltd,a_xyear,a_rev,a_divcode,a_plant,a_dvsn,p_mdcd,p_mdno,
      ...
      p_ygchk,p_orate,p_cumorate,p_serl,a_lastemp,a_lastdate,
      p_opcd,p_option,p_opchk);
