...
declare bomchk_cur02 cursor for
  select tplnt,tdvsn,tpitn,tcitn,twkct,tqty1,texplant,
         texdv,tchgchk,tserl
...
-- create qtemp.bomtemp01
 ...
 tchgchk char(1) not null,tserl varchar(600) not null);

delete from qtemp.bomtemp01;

set p_citno = trim(a_itno);
...
pdvsn = a_dvsn AND pcitn = p_citno AND
(( pedte = ' '  and pedtm <= a_date ) or
( pedte <> ' ' and pedtm <= a_date
                and pedte >= a_date ));
...
inc_loop:
loop
fetch bomchk_cur01 into p_plant,p_dvsn,p_pitno,p_wkct,
...
inc_loop01:
loop
 fetch bomchk_cur02 into p_plant,p_dvsn,p_pitno,p_citno,p_wkct,
