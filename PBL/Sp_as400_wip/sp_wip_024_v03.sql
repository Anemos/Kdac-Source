...
declare cur_wip009 cursor for
  ...
  from pbwip.wip009
  where wfyear = p_curryy and wfmonth = p_currmm and
    wfiocd = a_iocd;
...
update pbwip.wip002
  ...
  where wbcmcd  = a_cmcd and  wbiocd = a_iocd and
...
loop
  ...
  update pbwip.wip002
    set wbretn = p_retn, wbscrp = p_scrp,
      ...
      where wbcmcd  = a_cmcd and wbplant = p_plant and
            ...
            wbmonth = p_currmm and wbiocd = a_iocd;
  if retcode <> 0 then
    leave inc_loop;
  end if;
  -- update next month
  update pbwip.wip002
    set wbretn = p_retn, wbscrp = p_scrp,
      ...
      where wbcmcd  = a_cmcd and wbplant = p_plant and
            ...
            wbmonth = p_nextmm and wbiocd = a_iocd;
  if retcode <> 0 then
    leave inc_loop;
  end if;
  -- update wip001
  if p_phqt = 0 and p_ohqt < 0 then
    ...
    select a_cmcd,'WX', p_srno, p_plant, p_dvsn, a_iocd, p_itno,
    ...
    values ( a_cmcd, 'WX', p_srno, '사급정산 잉여재공이월') ;
    ...
    where wacmcd = a_cmcd and waplant = p_plant and
          ...
          waitno = p_itno and waiocd = a_iocd;
  else
    update pbwip.wip001
    set waretn = p_retn, wascrp = p_scrp,
        wabgqt = p_phqt, wabgat1 = p_phat,
