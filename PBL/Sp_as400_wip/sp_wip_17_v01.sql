...
loop
  ...
  if sqlcode <> 0 then
    ...
    while at_end = 0 do
      ...
      VALUES (a_year,a_month,a_cmcd,p_plant,p_dvsn,
          p_dept,' ',p_serialno,p_deptnm,' ',' ',p_chkpdcd,p_chkpdnm,
          ' ',0,0,'A',' ',' ',' ',' ',' ');
  ...
  SELECT COITNAME INTO p_deptnm
  FROM PBCOMMON.DAC002
  WHERE COMLTD = a_cmcd AND COGUBUN = 'DAC150' AND
        COCODE = p_dept;
  if sqlcode <> 0 then
    set p_dept = ' ';
    set p_deptnm = ' ';
  ...
  INSERT INTO PBWIP.WIP006(WFYEAR,WFMONTH,WFCMCD,WFPLANT,WFDVSN,
    ...
    WFINPTDT,WFUPDTDT)
  VALUES (a_year,a_month,a_cmcd,p_plant,p_dvsn,
    p_dept,p_itno,p_serialno,p_deptnm,p_itnm,p_spec,p_pdcd,p_pdnm,
    p_unit,0,0,'C',' ',' ',' ',' ',' ');
end loop;
close wip_001;
