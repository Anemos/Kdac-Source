--file name     : sp_wip_041.sql
--system                : kdac system
--procedure name        : pbwip.sp_wip_041
--initial       : 2003.02.10
--desc    :  WIP Vendor Detail Report
--author                : kim ki sub

drop procedure pbwip.sp_wip_041;

create procedure pbwip.sp_wip_041
(in @cmcd  char(2),
in @yyyy  char(4),
in @mm    char(2),
in @dd    char(2))
...
declare c1 cursor for
  select wbplant,wbdvsn,wborct,wbitno,wbpdcd,prname,wbdesc,wbitcl,
  ...
  where wbcmcd = comltd and wbpdcd = prprcd and
        wbcmcd = @cmcd and wbyear = @yyyy and
        wbmonth = @mm and wbiocd = '2' and
...
declare c2 cursor for
  select wdprno,itnm,wdprdpt,sum(wdprqt),sum(wdchqt)
  from pbwip.wip004, pbinv.inv002
  where wdcmcd = comltd and wdprno = itno and
        wdcmcd = @cmcd and wdplant = p_plant and
        wddvsn = p_dvsn and wdslty in ('WC','WR') and
        substr(wddate,1,6) = p_adjdt and
        wditno = p_itno and wdiocd = '2' and
        wdchdpt = p_orct
...
open c1;
fetch c1 into p_plant,p_dvsn,p_orct,p_itno,p_pdcd,p_pdnm,p_itnm,p_itcl,
	p_srce,p_unit,p_bgqt,p_inqt,p_usqt1,
	p_usqt2,p_usqt,p_ohqt;

while at_end = 0 do
-- check option item check
set p_ochk = pbpdm.f_bom_02(@cmcd,p_plant,p_dvsn,p_itno,
             @yyyy||@mm||@dd);

select convqty into p_convqty
 from pbinv.inv101
 where comltd = @cmcd and xplant = p_plant and
...
values (@cmcd,p_plant,p_dvsn,p_orct,p_itno,p_ochk,p_itnm,
