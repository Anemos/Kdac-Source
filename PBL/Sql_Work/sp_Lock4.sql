/*---------------------------------------------------------------------
  $Header: /abasql/aba_lockinfo_sqlmm.sp 13    06-03-21 21:09 Sommar $
  ...
  SQL6.5 and SQL7. If you are on SQL2000 SP3 or latter there is a better
  version.

  $History: aba_lockinfo_sqlmm.sp $
 * 
 * *****************  Version 13  *****************
 ...
 *
 * *****************  Version 12  *****************
 * User: Sommar       Date: 04-02-22   Time: 23:24
 * Updated in $/abasql
 ...
 * Created in $/projects/dbverktyg/abasql
  ---------------------------------------------------------------------*/
CREATE PROCEDURE sp_lock4 @processes tinyint = 0,
                              @details   bit     = 0,
                              @fancy     bit     = 0 AS
...
CREATE TABLE #result (
       ...
       PRIMARY KEY CLUSTERED (spid, ident))
...
INSERT @procs(spid, ecid, login, status,
              ...
              last_batch,
              last_since)
   SELECT p.spid, p.ecid, suser_sname(p.sid), rtrim(p.status),
...
-- Get input buffers. Note that only the main thread, ecid = 0 is of interest.
DECLARE C1 CURSOR LOCAL FOR
   SELECT str(spid), spid
   FROM   @procs
...
INSERT #objects (dbid, objid, indid)
   SELECT DISTINCT dbid, objid, indid
   FROM   @locks
