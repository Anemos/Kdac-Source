...
SET RENVMC = IFNULL((select sum(rfmctm)
	...
	RENVLB =  IFNULL((select sum(rflbtm)
		...
		WHERE rfcmcd = '01' AND rfplant = REPLANT AND rfdvsn = REDVSN AND
				rfitno = REITNO AND rfline1 = RELINE1 AND rfline2 = RELINE2 AND
				rfopno = REOPNO and RFEDTO = '99991231'),0)
WHERE REPLANT = 'D' AND REDVSN = 'S' AND REEDTO = '99991231';
...
SELECT * FROM PBRTN.RTN016 BB
WHERE BB.RFPLANT = 'D' AND BB.RFDVSN = 'S' AND BB.RFEDTO = '99991231' AND NOT EXISTS
( SELECT * FROM PBRTN.RTN014 AA
WHERE AA.RDPLANT = 'D' AND AA.RDDVSN = 'S' AND 
	AA.RDLOGID = BB.RFLOGID );

SELECT * FROM PBRTN.RTN014 BB
WHERE BB.RDPLANT = 'D' AND BB.RDDVSN = 'S'  AND NOT EXISTS
( SELECT * FROM PBRTN.RTN016 AA
WHERE AA.RFPLANT = 'D' AND AA.RFDVSN = 'S' AND AA.RFEDTO = '99991231' AND
...
//효율작업시간 상이 체크로직
SELECT A.RCCMCD, A.RCPLANT, A.RCDVSN, A.RCITNO, A.RCLINE1, A.RCLINE2, A.RCBLTM, A.RCNVLB, B.REBLTM, B.RENVLB 
FROM
( SELECT RCCMCD, RCPLANT, RCDVSN, RCITNO, RCLINE1, RCLINE2, SUM(RCBLTM) AS RCBLTM, SUM(RCNVLB) AS RCNVLB
FROM PBRTN.RTN013
WHERE RCCMCD = '01' AND RCPLANT = 'D' AND RCDVSN = 'S' AND RCFLAG <> 'D'
GROUP BY RCCMCD, RCPLANT, RCDVSN, RCITNO, RCLINE1, RCLINE2 ) A,

( SELECT RECMCD, REPLANT, REDVSN, REITNO, RELINE1, RELINE2, SUM(REBLTM) AS REBLTM, SUM(RENVLB) AS RENVLB
FROM PBRTN.RTN015
WHERE RECMCD = '01' AND REPLANT = 'D' AND REDVSN = 'S' AND REEDTO = '99991231'
GROUP BY RECMCD, REPLANT, REDVSN, REITNO, RELINE1, RELINE2 ) B
WHERE A.RCCMCD = B.RECMCD AND A.RCPLANT = B.REPLANT AND A.RCDVSN = B.REDVSN AND
	A.RCITNO = B.REITNO AND A.RCLINE1 = B.RELINE1 AND A.RCLINE2 = B.RELINE2;