...
decimal(CASE WHEN cc.srce NOT IN ('05','03',' ') THEN cc.costls
  ELSE 0.000000 END,15,2) as costls,
decimal(CASE WHEN cc.srce NOT IN ('05','03',' ') THEN
  CASE WHEN cc.costav = 0 THEN cc.costls ELSE cc.costav END
  ELSE 0.000000 END,15,2) as costav,
decimal(CASE WHEN cc.srce NOT IN ('05','03',' ') THEN
  CASE WHEN cc.outqty <> 0
       THEN abs(cc.OUTAMT / cc.OUTQTY)
       ELSE cc.COSTLS END
  ELSE 0.000000 END,15,2) as outcost,
cc.xunit,
  tmp.wbsrce, tmp.inqt, aa.vsrno, bb.vndnm, tmp.wborct, tmp.vndnm, ff.pqtym, dd.xunit,
	ff.wdprno, ff.itnm, ff.spec, ff.vsrno, ff.vndnm, ff.xplan, ff.coitname, gg.costls
from ( select aa.wbcmcd as wbcmcd,aa.wbplant as wbplant,aa.wbdvsn as wbdvsn,
  ...
  where aa.wbcmcd = '01' and aa.wbyear >= '2009' and aa.wbyear <= '2009' and aa.wbmonth = '04' and
        aa.wbiocd = '2' and aa.wbplant = 'D' and aa.wbdvsn = 'H' and
  ...
  aa.wborct ,bb.vndnm ,aa.wbitno ,aa.wbdesc ,
	...
	left outer join pbinv.inv402 cc
	on cc.comltd = '01' and cc.xplant = tmp.wbplant and
		cc.div = tmp.wbdvsn and cc.itno = tmp.wbitno and cc.xyear = '200904'
	left outer join pbinv.inv002 dd
	on dd.comltd = '01' and dd.itno = tmp.wbitno
	left outer join ( select aa.wdcmcd as wdcmcd,aa.wdplant as wdplant,aa.wddvsn as wddvsn,
		...
		from (select distinct wdcmcd,wdplant,wddvsn, wditno, wdchdpt, wdprno
			...
			wddvsn = 'H' and wddate >= '20090401' and wddate <= '20090430') aa
	...
	inner join pbinv.inv402 gg
	on ff.wdcmcd = gg.comltd and ff.wdplant = gg.xplant and
	  ff.wddvsn = gg.div and ff.wdprno = gg.itno and
	  gg.xyear = '200904'
  ;
