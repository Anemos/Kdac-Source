/*
  ...
  use table :
  initial   : 2002.12
  desc : 기준일자부터 예방정비계획이 생성되도록 프로그램 변경(2012.04.20)
*/
...
BEGIN -- BEGIN PROCEDURE
  ...
  BEGIN
    ...
    WHILE (@NEXT_DATE > @PS_YYMMDD_A)
    BEGIN
      SELECT @NEXT_DATE =
          CASE @CYCLE_CODE
            WHEN '일' THEN DATEADD(DAY, -1 * @CLASS_CYCLE, @NEXT_DATE)
            WHEN '주' THEN DATEADD(WEEK, -1 * @CLASS_CYCLE, @NEXT_DATE)
            WHEN '월' THEN DATEADD(MONTH, -1 * @CLASS_CYCLE, @NEXT_DATE)
            WHEN '분기' THEN DATEADD(MONTH, -1 * @CLASS_CYCLE, @NEXT_DATE)
            WHEN '반기' THEN DATEADD(MONTH, -1 * @CLASS_CYCLE, @NEXT_DATE)
            WHEN '년' THEN DATEADD(YEAR, -1 * @CLASS_CYCLE, @NEXT_DATE)
