/*
  file name : sp_create_task.sql
  system    : cmms system
  procedure name  : sp_create_task
  description :
  use database  : cmms
  use program :
  parameter :
...
if exists (select * from sysobjects
      where id = object_id(N'[dbo].[sp_create_task]')
        and objectproperty(id, N'isprocedure') = 1)
  drop procedure [dbo].[sp_create_task]
go
...
Declare   @li_TotCnt    AS Int
  ...
  From Equip_Class A inner join Equip_Master B
    on A.area_code = B.area_code and B.factory_code = B.factory_code and A.equip_code = B.equip_code
  Where B.Area_Code = @LS_area And B.Factory_Code = @LS_factory And B.Equip_Code = @LS_equip_code
      And Convert(VarChar(10), A.Class_Est_Date, 120) <= Convert(VarChar(10), DateAdd(dd, 10, GetDate()), 120)
      And B.Equip_Div_Code Not IN ('3', '8', '9', 'X') And A.Class_Div <> '04'
