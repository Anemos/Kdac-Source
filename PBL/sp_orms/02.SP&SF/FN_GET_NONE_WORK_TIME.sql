...
BEGIN
  ...
  SELECT @li_NONE_TIME = ISNULL(SUM(DATEDIFF(MI,CAST( SUBSTRING(CONVERT(VARCHAR(20),GETDATE(), 120), 1, 11) + LEFT(CDNAME,5) + ':00.000'  AS DATETIME),
          CAST( SUBSTRING(CONVERT(VARCHAR(20),GETDATE(),120), 1, 11) + RIGHT(CDNAME,5) + ':59.999' AS DATETIME)) ),0)
	    FROM TM_CODE
	    WHERE MCD		 	= 
			( SELECT CDNAME
			   FROM TM_CODE
  ...
  IF EXISTS ( 
	    ...
	    BEGIN
	      ...
	      RETURN DATEDIFF(MI,CAST( SUBSTRING(CONVERT(VARCHAR(20),GETDATE(), 120), 1, 11) + @ls_STD_HHMM + ':00.000'  AS DATETIME),
          CAST( SUBSTRING(CONVERT(VARCHAR(20),GETDATE(),120), 1, 11) + @ls_NONE_HHMM_MAX + ':59.999' AS DATETIME))
	    END
	 ...
	 IF EXISTS ( 
	    ...
	    BEGIN
	      ...
	      RETURN DATEDIFF(MI,CAST( SUBSTRING(CONVERT(VARCHAR(20),GETDATE(), 120), 1, 11) + @ls_NONE_HHMM_MIN + ':00.000'  AS DATETIME),
          CAST( SUBSTRING(CONVERT(VARCHAR(20),GETDATE(),120), 1, 11) + @ls_END_HHMM + ':59.999' AS DATETIME))
	    END
