-- file name : pbpdm.sp_bom_101
-- procedure name : pbpdm.sp_bom_101
-- desc : Creation BOM109, 고객사유상사급제외 재료비
-- a_chk : FULL ('A'), 전체 ('B'), 공장별('C') to 'E'
--         Create BOM113 'K'
--         Create BOM115 'M'

drop procedure pbpdm.sp_bom_101;
create procedure pbpdm.sp_bom_101 (
in a_comltd char(2),
in a_plant char(1),
in a_dvsn char(1),
...
loop
  ...
  if a_chk = 'A' then
    -- First Step
    set p_exprtn = pbpdm.sf_bom_t01(a_comltd,p_plant,p_dvsn,
    ...
    set p_rtn = pbpdm.sf_bom_103(a_comltd,p_plant,p_dvsn,
      p_itno,p_pdcd,p_costdiv,p_yyyymm,a_createdate,p_exprtn);
    call pbpdm.sp_bom_104(a_comltd,p_plant,p_dvsn,p_pdcd,a_applydate);
    ...
    set p_rtn = pbpdm.sf_bom_104(a_comltd,p_plant,p_dvsn,
      p_itno,p_pdcd,p_costdiv,p_yyyymm,a_createdate,p_exprtn);
    call pbpdm.sp_bom_103(a_comltd,p_plant,p_dvsn,p_pdcd,a_applydate);
  end if;
  if a_chk = 'B' then
    -- First Step
    set p_exprtn = pbpdm.sf_bom_t01(a_comltd,p_plant,p_dvsn,
