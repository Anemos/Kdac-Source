-- file name : pbpdm.sp_bom_106
-- procedure name : pbpdm.sp_bom_106
-- desc : BOM113D, BOM IPIS 인터페이스 전개내역 테이블

drop procedure pbpdm.sp_bom_106;
create procedure pbpdm.sp_bom_106 (
in a_comltd char(2),
in a_plant char(1),
in a_dvsn char(1),
in a_pdcd varchar(4),
...
loop
 ...
 fetch first 1 row only;
  ...
  when tlevel = 7 then concat('.......',cast(tlevel as varchar(3)))
    ...
    c.xunit,c.srce,c.abccd,c.iscd,c.cls,c.puls,a.tlevel,
    ...
    a.tedte,tcalculate,p_explant,p_exdiv,topchk,tsubpaycd,ifnull(p_ygcst,0),
