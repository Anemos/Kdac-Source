...
loop
 ...
 into p_plant,p_dvsn,p_tcitn,
    p_tserl,p_srce,p_inputcost,p_ygchk
 ...
 where a.tcmcd = a_comltd and a.tplnt = a_plant and
   a.tdvsn = a_dvsn and a.topchk <> '2' and
   a.tserl > p_tserl
 order by a.tserl
 fetch first 1 row only;
  ...
  if p_srce = '01' then
    ...
    from pbpur.opm102
    where comltd = a_comltd and itno = p_tcitn and
      inptdt <= a_applydate
  ...
  if p_srce = '02' or p_srce = '04' then
    ...
    where a.comltd = a_comltd and a.itno = p_tcitn and
      a.pdudt <= a_applydate
    order by a.pdudt desc
    fetch first 1 row only;
  ...
  ( zcmcd,zdate,zplant,zdiv,zmdcd,zmdno,
    ...
    ztodt,zcalc,zcomcd,zygchk,zygcst,zsubpaycd,
  ...
  when tlevel = 7 then concat('.......',cast(tlevel as varchar(3)))
    ...
    a.tedte,tcalculate,tcomcd,p_ygchk,ifnull(p_ygcst,0),tsubpaycd,
