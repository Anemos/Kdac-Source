-- file name : sf_bom_exm
-- procedure name : pbpdm.sf_bom_exm
-- desc : bom explosion for inv402 이월기준
...
drop function pbpdm.sf_bom_exm;
create function pbpdm.sf_bom_exm (
a_comltd varchar(2),
a_plant varchar(1),
a_dvsn varchar(1),
a_itno   varchar(15),
...
declare p_calculate2  char(1);
declare p_parentcal2  char(1);
...
select a.plant,a.pdvsn,a.pcitn,a.pwkct,a.poscd,a.pedtm,a.pedte,
 a.pqtym,a.pexplant,a.pexdv,c.cls,c.srce,
...
declare bomchk_cur02 cursor for
  select tplnt,tdvsn,tcitn,twkct,tqty1,texplant,
         texdv,topchk,tserl,tcalculate,tsubpaycd,tcalculate2
  from qtemp.tmp_bom
...
select srce, cls, comcd, case when convqty = 0 then 1 else convqty end
into p_srce, p_itcl, p_comcd, p_convqty
from pbinv.inv101
...
-- insert zero level not applying conversion factor
...
if sqlcode <> 0 or p_chkcnt < 1 then
  ...
  if sqlcode <> 0 then
    ...
    values(a_comltd,a_plant,a_dvsn,p_pitno,
       ...
       '','',p_opcd,'','','','Y',
       p_comcd,'000',
       decimal(truncate(p_qty * p_outcost,3),10,3),
       decimal(truncate(p_qty * p_movecost,3),10,3),
       decimal(truncate(p_qty * p_inputcost,3),10,3),
       p_outcost,p_movecost,p_inputcost,
       decimal(truncate(p_qty * p_scoutcost,3),10,3),
...
if p_srce = '01' or p_srce = '02' or p_srce = '04' then
 if p_ygchk <> 'Y' then
  if p_inputcost = 0 and p_movecost = 0 and p_outcost = 0 then
    set p_inputcost = pbpur.f_getcost3(p_srce,p_pitno);
  ...
  if p_scinputcost = 0 and p_scmovecost = 0 and p_scoutcost = 0 then
    set p_scinputcost = pbpdm.sf_bom_106(p_pitno);
...
if p_srce = '03' and p_wkct = '8888' then
  set p_calculate2 = 'Y';
else
  set p_calculate2 = 'N';
end if;
...
if p_srce = '01' or p_srce = '02' or p_srce = '04' then
 if p_ygchk <> 'Y' then
  if p_inputcost = 0 and p_movecost = 0 and p_outcost = 0 then
    set p_inputcost = pbpur.f_getcost3(p_srce,p_citno);
  ...
  if p_scinputcost = 0 and p_scmovecost = 0 and p_scoutcost = 0 then
    set p_scinputcost = pbpdm.sf_bom_106(p_citno);
...
loop
 ...
 if p_parentcal2 = 'Y' or (p_wkct = '8888' and p_srce = '03') then
   set p_calculate2 = 'Y';
 else
   set p_calculate2 = 'N';
 end if;
