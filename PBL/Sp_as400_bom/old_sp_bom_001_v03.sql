...
if a_chk <> 'A' and a_chk <> 'C' and a_chk <> 'K' and a_chk <> 'M' then
  return;
end if;
...
loop
  ...
  if a_chk = 'A' then
    ...
    set p_rtn = pbpdm.sf_bom_002(a_comltd,p_plant,p_dvsn,
      p_itno,p_pdcd,p_costdiv,p_yyyymm,a_createdate,p_exprtn);
    if p_exprtn <> 'N' then
      call pbpdm.sp_bom_003(a_comltd,p_plant,p_dvsn,p_pdcd,a_applydate);
      call pbpdm.sp_bom_004(a_comltd,p_plant,p_dvsn,p_pdcd,a_applydate);
    end if;
  end if;
  if a_chk = 'C' then
    set p_rtn = pbpdm.sf_bom_002(a_comltd,p_plant,p_dvsn,
  ...
  if a_chk = 'K' and p_exprtn <> 'N' then
    call pbpdm.sp_bom_003(a_comltd,p_plant,p_dvsn,p_pdcd,a_applydate);
  end if;
  if a_chk = 'M' and p_exprtn <> 'N' then
    call pbpdm.sp_bom_004(a_comltd,p_plant,p_dvsn,p_pdcd,a_applydate);
  end if;
end loop;
close bom_001;
